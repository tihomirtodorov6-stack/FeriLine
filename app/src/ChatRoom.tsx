...
export default function ChatRoom({ name, contact, onBack }: any) {
  ...
  async function startCall(){
    try{
      ...
      localStreamRef.current = stream; setIsMuted(false);
    ...
    }catch(err){ alert('Трябва да разрешиш микрофона! Сайтът трябва да е на https'); }
  }
  ...
  async function sendMessage() {
    ...
    if (data) { setText(""); setMessages((o:any)=>[...o, data]); }
  }
  ...
  return (
    <div className="chat-room" style={{position:'relative'}}>
      ...
      <div className="messages">
        {messages.map((msg:any) => (<div key={msg.id} className={msg.sender_id === currentUser.id? "message mine" : "message"}>{msg.text}</div>))}
        <div ref={bottomRef} />
      </div>
