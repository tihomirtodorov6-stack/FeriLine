...
export default function Home(){
  ...
  const callAdmin = async (action:string, data:any={})=>{
    const res = await fetch(EDGE_URL, {
      ...
      body: JSON.stringify({action,...data, adminPhone: currentUser?.phone, phone: currentUser?.phone})
    });
  ...
  const loadRides = async ()=>{
    ...
    const {data} = await supabase.from('rides').select('*').gte('created_at', cutoffIso).order('created_at',{ascending:true});
  ...
  // ====== РЕАЛНА РЕГИСТРАЦИЯ / ВХОД СЪС SUPABASE USERS ТАБЛИЦА ======
  const handleAuth = async ()=>{
    ...
    if(loginTab==='register'){
      ...
      if(existing){ alert('Този телефон вече е регистриран! Влез с Вход.'); setLoginTab('login'); return; }
      ...
      if(!found){ alert('Няма акаунт с този телефон! Първо се регистрирай.'); setLoginTab('register'); return; }
  ...
  const handleDeleteAccount = async ()=>{ if(!confirm('Сигурен ли си? Ще изтрие акаунта и пътуванията ЗАВИНАГИ!')) return; try{ await supabase.from('rides').delete().eq('driver_id', currentUser.id); await supabase.from('users').delete().eq('id', currentUser.id); }catch{} localStorage.removeItem('vozime_current'); setCurrentUser(null); };
  ...
  const publishRide= async ()=>{
    ...
    const existingCreated = editingRide? (rides.find(r=>r.id===editingRide)?.createdAt || new Date().toISOString()) : new Date().toISOString();
    ...
    try{
      if(editingRide){ await callAdmin('update_ride',{ride_id:editingRide, newData:row}); }
  ...
  const handleReport = async (ride:any)=>{
    ...
    setReports([ {...newReport, reportedPhone:newReport.reported_phone, reportedName:newReport.reported_name, from:newReport.from_city, to:newReport.to_city, reporterPhone:newReport.reporter_phone, reporterName:newReport.reporter_name, createdAt:newReport.created_at} as any,...reports]); alert('Доклад изпратен!');
  };
  ...
  if(maintenance.enabled &&!isAdmin){
    return (<main style={{height:'100dvh',display:'flex',alignItems:'center',justifyContent:'center',background:'#0F4C75',color:'white',padding:'20px',textAlign:'center'}}><div><div style={{fontSize:'60px'}}>🔧</div><div style={{fontSize:'24px',fontWeight:'bold',marginTop:'10px'}}>{t.maintenanceTitle}</div><div style={{fontSize:'14px',marginTop:'10px'}}>{maintenance.msg||t.maintenanceMsg}</div></div></main>);
  }
  ...
  if(!currentUser){
    return (
      <main style={{position:'fixed',inset:0,display:'flex',alignItems:'center',justifyContent:'center',background:'#0F4C75',padding:'16px',boxSizing:'border-box'}}>
        <div style={{background:'white',padding:'20px',borderRadius:'24px',width:'100%',maxWidth:'380px',boxSizing:'border-box',boxShadow:'0 20px 40px rgba(0,0,0,0.25)'}}>
          ...
          <div style={{display:'flex',background:'#f1f3f4',borderRadius:'12px',padding:'3px',marginBottom:'16px'}}>
            <button onClick={()=>setLoginTab('login')} style={{flex:1,padding:'10px',borderRadius:'8px',border:'none',fontWeight:'bold',background:loginTab==='login'?'#0F4C75':'white',color:loginTab==='login'?'white':'#666',fontSize:'13px'}}>{t.tabLogin}</button>
            <button onClick={()=>setLoginTab('register')} style={{flex:1,padding:'10px',borderRadius:'8px',border:'none',fontWeight:'bold',background:loginTab==='register'?'#0F4C75':'white',color:loginTab==='register'?'white':'#666',fontSize:'13px'}}>{t.tabReg}</button>
          </div>
          <input placeholder={t.firstName} value={loginForm.firstName} onChange={e=>setLoginForm({...loginForm,firstName:e.target.value})} style={{width:'100%',padding:'14px 16px',borderRadius:'12px',border:'1px solid #e5e7eb',background:'#f9fafb',boxSizing:'border-box',marginBottom:'10px',outline:'none'}}/>
          <input placeholder={t.lastName} value={loginForm.lastName} onChange={e=>setLoginForm({...loginForm,lastName:e.target.value})} style={{width:'100%',padding:'14px 16px',borderRadius:'12px',border:'1px solid #e5e7eb',background:'#f9fafb',boxSizing:'border-box',marginBottom:'10px',outline:'none'}}/>
          <input placeholder={t.phone} value={loginForm.phone} onChange={e=>setLoginForm({...loginForm,phone:e.target.value})} style={{width:'100%',padding:'14px 16px',borderRadius:'12px',border:'1px solid #e5e7eb',background:'#f9fafb',boxSizing:'border-box',marginBottom:'16px',outline:'none'}}/>
          <button onClick={handleAuth} style={{width:'100%',padding:'16px',background:'#3DD68C',border:'none',borderRadius:'14px',fontWeight:'bold',color:'#0F62FE',fontSize:'15px',cursor:'pointer'}}>{loginTab==='login'?t.loginBtn:t.regBtn}</button>
        </div>
      </main>
    );
  }
  ...
  return (
    <main style={{position:'fixed',inset:0,width:'100%',maxWidth:'480px',margin:'0 auto',background:'white',display:'flex',flexDirection:'column',overflow:'hidden',fontFamily:'-apple-system, sans-serif',boxSizing:'border-box'}}>
      <div style={{flexShrink:0}}>
        <div style={{background:maintenance.enabled?'#FF3B30':'#0F4C75',color:'white',padding:'6px',textAlign:'center',fontSize:'10px',boxSizing:'border-box'}}>{maintenance.enabled?'🔴 MAINTENANCE - Сайтът е спрян!':'🌍 VoziMe • 100% безплатна платформа • Сигурна ✅'}</div>
      ...
      {tab==='find' && (<div style={{flexShrink:0,background:'white',padding:'8px 10px',borderBottom:'1px solid #e5e7eb',display:'flex',flexDirection:'column',gap:'8px',boxSizing:'border-box'}}><div style={{background:'#e3f2fd',padding:'10px',borderRadius:'12px',border:'1px solid #90caf9'}}><div style={{fontSize:'11px',fontWeight:'800',color:'#0F4C75'}}>{t.important}</div><div style={{fontSize:'11px',marginTop:'4px'}}><b>{t.important1}</b> {t.important1b}<br/><b>{t.important2}</b> {t.important2b}</div></div><div style={{display:'flex',gap:'6px'}}><button onClick={()=>setFilterType('')} style={{flex:1,padding:'8px',borderRadius:'8px',border:'none',fontWeight:'bold',fontSize:'11px',background:filterType===''?'#0F4C75':'white',color:filterType===''?'white':'#666'}}>{t.all}</button><button onClick={()=>setFilterType('offer')} style={{flex:1,padding:'8px',borderRadius:'8px',border:'none',fontWeight:'bold',fontSize:'10px',background:filterType==='offer'?'#2ECC71':'white'}}>🚗 {t.drivers}</button><button onClick={()=>setFilterType('request')} style={{flex:1,padding:'8px',borderRadius:'8px',border:'none',fontWeight:'bold',fontSize:'10px',background:filterType==='request'?'#FFD60A':'white'}}>🙋 {t.passengers}</button></div><input placeholder={t.search} value={filterText} onChange={e=>setFilterText(e.target.value)} style={{padding:'10px',borderRadius:'10px',border:'1px solid #ddd',fontSize:'13px',width:'100%',boxSizing:'border-box'}}/></div>)}
      <div style={{flex:1,overflowY:'auto',background:'#f9fafb',boxSizing:'border-box'}} ref={ridesContainerRef}>
        {tab==='find' && (<div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'10px'}}>{filteredRides.map((r:any)=>{ const banned = isBanned(r.driverPhone); const isReq = r.type==='request'; return (<div key={r.id} style={{border: banned?'2px solid #FF3B30':isReq?'2px solid #FFD60A':'2px solid #2ECC71',borderRadius:'14px',padding:'12px',background: banned?'#ffeaea':isReq?'#fffbe6':'#f0fdf4',opacity:banned?0.6:1,boxSizing:'border-box'}}>{banned && <div style={{background:'#FF3B30',color:'white',fontSize:'10px',padding:'4px 8px',borderRadius:'6px',marginBottom:'6px',fontWeight:'bold'}}>⛔ БАННАТ: {banned.reason}</div>}<div style={{display:'flex',justifyContent:'space-between'}}><b style={{fontSize:'14px'}}>{getFlag(r.fromCountry)} {r.from} → {getFlag(r.toCountry)} {r.to}</b><span style={{background:isReq?'#FFD60A':'#2ECC71',color:isReq?'black':'white',fontSize:'9px',padding:'3px 8px',borderRadius:'12px',fontWeight:'bold'}}>{isReq?`🙋 ${t.seeks}`:`🚗 ${t.offers}`}</span></div><div style={{fontSize:'11px',marginTop:'4px'}}>{r.driverName} • {r.seats} • {r.carInfo} • {r.time}</div><div style={{fontSize:'10px',background:'#fff3cd',padding:'6px 8px',borderRadius:'8px',marginTop:'6px',border:'1px solid #ffe69c'}}>{t.notFree}</div>{r.message && <div style={{fontSize:'11px',marginTop:'6px',background:'white',padding:'6px',borderRadius:'6px'}}>💬 {r.message}</div>}<div style={{marginTop:'10px',background:'white',borderRadius:'10px',padding:'8px',border:'1px solid #ddd'}}><div style={{fontSize:'10px',fontWeight:'bold',color:'#0F4C75',marginBottom:'6px'}}>{t.connectDirect}</div><div style={{fontSize:'12px',fontWeight:'bold',marginBottom:'8px'}}>{r.driverName} • {r.driverPhone}</div><div style={{display:'flex',gap:'5px',marginBottom:'6px'}}><a href={`tel:${cleanPhone(r.driverPhone)}`} style={{flex:1,background:'#0F4C75',color:'white',padding:'10px',borderRadius:'8px',textAlign:'center',fontWeight:'bold',textDecoration:'none',fontSize:'11px'}}>{t.call}</a><a href={`https://wa.me/${waPhone(r.driverPhone)}`} target="_blank" style={{flex:1,background:'#25D366',color:'white',padding:'10px',borderRadius:'8px',textAlign:'center',fontWeight:'bold',textDecoration:'none',fontSize:'11px'}}>WhatsApp</a><a href={`viber://chat?number=${encodeURIComponent(cleanPhone(r.driverPhone))}`} style={{flex:1,background:'#7360F2',color:'white',padding:'10px',borderRadius:'8px',textAlign:'center',fontWeight:'bold',textDecoration:'none',fontSize:'11px'}}>Viber</a></div><div style={{display:'flex',gap:'5px'}}><button onClick={()=>handleReport(r)} style={{flex:1,background:'#fff3cd',border:'1px solid #ffe69c',padding:'6px',borderRadius:'6px',fontSize:'10px',fontWeight:'bold'}}>{t.report}</button>{isAdmin && <button onClick={()=>deleteRide(r.id)} style={{background:'#FF3B30',color:'white',border:'none',padding:'6px 12px',borderRadius:'6px',fontSize:'10px'}}>🗑️</button>}{isAdmin && <button onClick={()=>{const reason=prompt('Причина:'); if(reason) handleBan(r.driverPhone, reason, '24h');}} style={{background:'black',color:'white',border:'none',padding:'6px 12px',borderRadius:'6px',fontSize:'10px'}}>⛔ БАН</button>}</div></div></div>)})}</div>)}
        {tab==='admin' && isAdmin && (<div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'12px',boxSizing:'border-box'}}><div style={{background:'#0F4C75',color:'white',padding:'12px',borderRadius:'12px'}}><div style={{fontWeight:'bold',fontSize:'14px'}}>👑 АДМИН • Сигурен ✅</div><div style={{fontSize:'11px',opacity:0.8}}>Service Role + RLS + CORS ✅</div></div><div style={{background:'white',border:'2px solid #FF3B30',borderRadius:'12px',padding:'12px'}}><div style={{fontWeight:'bold',fontSize:'13px',color:'#FF3B30'}}>🔧 СПРИ САЙТА</div><div style={{display:'flex',gap:'8px',marginTop:'10px',alignItems:'center'}}><button onClick={toggleMaintenance} style={{background:maintenance.enabled?'#2ECC71':'#FF3B30',color:'white',border:'none',padding:'10px 16px',borderRadius:'8px',fontWeight:'bold',fontSize:'12px'}}>{maintenance.enabled?'✅ СПРЯН - ПУСНИ':'🔴 СПРИ САЙТА СЕГА'}</button><span style={{fontSize:'11px',fontWeight:'bold',color:maintenance.enabled?'#FF3B30':'#2ECC71'}}>{maintenance.enabled?'🔴 СПРЯН':'🟢 Работи'}</span></div><input placeholder="Съобщение..." value={maintenance.msg} onChange={e=>updateMaintenanceMsg(e.target.value)} style={{width:'100%',marginTop:'8px',padding:'8px',borderRadius:'8px',border:'1px solid #ddd',fontSize:'11px',boxSizing:'border-box'}}/></div><div style={{background:'white',border:'2px solid black',borderRadius:'12px',padding:'12px'}}><div style={{fontWeight:'bold',fontSize:'13px'}}>⛔ БАН - {bans.length}</div><div style={{display:'flex',gap:'6px',marginTop:'8px'}}><input placeholder="Телефон" value={banPhoneInput} onChange={e=>setBanPhoneInput(e.target.value)} style={{flex:1,padding:'8px',borderRadius:'8px',border:'1px solid #ddd',fontSize:'11px',boxSizing:'border-box'}}/><select value={banDuration} onChange={e=>setBanDuration(e.target.value)} style={{padding:'8px',borderRadius:'8px',border:'1px solid #ddd',fontSize:'11px'}}><option value="1h">1ч</option><option value="24h">24ч</option><option value="7d">7д</option><option value="forever">Завинаги</option></select></div><input placeholder="Причина" value={banReasonInput} onChange={e=>setBanReasonInput(e.target.value)} style={{width:'100%',marginTop:'6px',padding:'8px',borderRadius:'8px',border:'1px solid #ddd',fontSize:'11px',boxSizing:'border-box'}}/><button onClick={()=>{if(!banPhoneInput) return; handleBan(banPhoneInput, banReasonInput||'Некоректен', banDuration);}} style={{width:'100%',marginTop:'6px',background:'black',color:'white',padding:'10px',borderRadius:'8px',fontWeight:'bold',border:'none'}}>⛔ БАННИ ГЛОБАЛНО</button><div style={{marginTop:'10px',display:'flex',flexDirection:'column',gap:'6px',maxHeight:'200px',overflowY:'auto'}}>{bans.map(b=><div key={b.id} style={{background:'#ffeaea',padding:'8px',borderRadius:'8px',display:'flex',justifyContent:'space-between',boxSizing:'border-box'}}><div><div style={{fontWeight:'bold',fontSize:'11px'}}>{b.originalPhone||b.original_phone} ({b.phone})</div><div style={{fontSize:'10px'}}>{b.reason} • {b.until==='forever'?'завинаги':new Date(Number(b.until)).toLocaleString()}</div></div><button onClick={()=>unban(b.id)} style={{background:'#2ECC71',border:'none',padding:'6px 10px',borderRadius:'6px',fontSize:'10px'}}>UNBAN</button></div>)}</div></div><div style={{background:'white',border:'2px solid #FFD60A',borderRadius:'12px',padding:'12px'}}><div style={{fontWeight:'bold',fontSize:'13px'}}>🚩 ДОКЛАДИ - {reports.length}</div><div style={{marginTop:'10px',display:'flex',flexDirection:'column',gap:'8px',maxHeight:'400px',overflowY:'auto'}}>{reports.map(rep=><div key={rep.id} style={{background:'#fffbe6',border:'1px solid #FFD60A',padding:'10px',borderRadius:'10px',boxSizing:'border-box'}}><div style={{fontSize:'11px',fontWeight:'bold'}}>🚩 {rep.reportedName||rep.reported_name} • {rep.reportedPhone||rep.reported_phone} • {rep.from||rep.from_city}→{rep.to||rep.to_city}</div><div style={{fontSize:'10px',marginTop:'2px'}}>Причина: <b>{rep.reason}</b></div><div style={{fontSize:'10px',color:'#666'}}>От: {rep.reporterName||rep.reporter_name} ({rep.reporterPhone||rep.reporter_phone})</div><div style={{display:'flex',gap:'6px',marginTop:'6px'}}><button onClick={()=>banFromReport(rep,'24h')} style={{flex:1,background:'#FF3B30',color:'white',border:'none',padding:'6px',borderRadius:'6px',fontSize:'10px',fontWeight:'bold'}}>⛔ БАН 24ч</button><button onClick={()=>banFromReport(rep,'forever')} style={{flex:1,background:'black',color:'white',border:'none',padding:'6px',borderRadius:'6px',fontSize:'10px',fontWeight:'bold'}}>⛔ Завинаги</button><button onClick={()=>dismissReport(rep.id)} style={{padding:'6px 10px',borderRadius:'6px',border:'1px solid #ddd',background:'white',fontSize:'10px'}}>❌</button></div></div>)}{reports.length===0 && <div style={{fontSize:'11px',textAlign:'center',color:'#666'}}>Няма доклади</div>}</div></div></div>)}
        {tab==='offer' && (<div style={{padding:'12px',display:'flex',flexDirection:'column',gap:'10px',boxSizing:'border-box'}}>{currentUser && isBanned(currentUser.phone) && <div style={{background:'#FF3B30',color:'white',padding:'10px',borderRadius:'10px',fontWeight:'bold',fontSize:'12px'}}>⛔ БАННАТ СИ: {isBanned(currentUser.phone)?.reason}</div>}<div style={{display:'flex',gap:'6px',background:'#f1f3f4',padding:'3px',borderRadius:'12px'}}><button onClick={()=>setOfferForm({...offerForm,type:'offer'})} style={{flex:1,padding:'10px',borderRadius:'8px',border:'none',fontWeight:'bold',fontSize:'11px',background:offerForm.type==='offer'?'#0F4C75':'white',color:offerForm.type==='offer'?'white':'#666'}}>{t.imDriver}</button><button onClick={()=>setOfferForm({...offerForm,type:'request'})} style={{flex:1,padding:'10px',borderRadius:'8px',border:'none',fontWeight:'bold',fontSize:'11px',background:offerForm.type==='request'?'#FFD60A':'white',color:offerForm.type==='request'?'black':'#666'}}>{t.imPassenger}</button></div><div style={{display:'flex',gap:'6px'}}><select value={offerForm.fromCountry} onChange={e=>setOfferForm({...offerForm,fromCountry:e.target.value})} style={{padding:'10px',borderRadius:'10px',border:'2px solid #2ECC71',background:'#e6f9ed',fontWeight:'bold',fontSize:'12px'}}>{COUNTRIES.map(c=><option key={c.code} value={c.code}>{c.name}</option>)}</select><input placeholder={`${t.from} - London`} value={offerForm.from} onChange={e=>setOfferForm({...offerForm,from:e.target.value})} style={{flex:1,padding:'10px',borderRadius:'10px',border:offerForm.from?'2px solid #2ECC71':'2px solid #FF3B30',boxSizing:'border-box'}}/></div><div style={{display:'flex',gap:'6px'}}><select value={offerForm.toCountry} onChange={e=>setOfferForm({...offerForm,toCountry:e.target.value})} style={{padding:'10px',borderRadius:'10px',border:'2px solid #2ECC71',background:'#e6f9ed',fontWeight:'bold',fontSize:'12px'}}>{COUNTRIES.map(c=><option key={c.code} value={c.code}>{c.name}</option>)}</select><input placeholder={`${t.to} - Sofia`} value={offerForm.to} onChange={e=>setOfferForm({...offerForm,to:e.target.value})} style={{flex:1,padding:'10px',borderRadius:'10px',border:offerForm.to?'2px solid #2ECC71':'2px solid #FF3B30',boxSizing:'border-box'}}/></div>{offerForm.type==='offer' && <><div style={{display:'flex',gap:'6px'}}><input placeholder={`${t.brand} *`} value={offerForm.carBrand} onChange={e=>setOfferForm({...offerForm,carBrand:e.target.value})} style={{flex:1,padding:'10px',borderRadius:'10px',border:offerForm.carBrand?'2px solid #2ECC71':'2px solid #FF3B30',boxSizing:'border-box'}}/><input placeholder={`${t.color} *`} value={offerForm.carColor} onChange={e=>setOfferForm({...offerForm,carColor:e.target.value})} style={{flex:1,padding:'10px',borderRadius:'10px',border:offerForm.carColor?'2px solid #2ECC71':'2px solid #FF3B30',boxSizing:'border-box'}}/></div><input placeholder={`${t.reg} *`} value={offerForm.carReg} onChange={e=>setOfferForm({...offerForm,carReg:e.target.value.toUpperCase()})} style={{width:'100%',padding:'10px',borderRadius:'10px',border:offerForm.carReg?'2px solid #2ECC71':'2px solid #FF3B30',boxSizing:'border-box'}}/></>}<input placeholder={offerForm.type==='request'?`${t.seatsNeed} - 2`:`${t.seatsFree} - 4`} value={offerForm.seats} onChange={e=>setOfferForm({...offerForm,seats:e.target.value})} style={{padding:'10px',borderRadius:'10px',border:offerForm.seats?'2px solid #2ECC71':'2px solid #FF3B30',boxSizing:'border-box'}}/><textarea placeholder={t.note} value={offerForm.message} onChange={e=>setOfferForm({...offerForm,message:e.target.value})} style={{padding:'10px',borderRadius:'10px',border:'1px solid #ddd',minHeight:'50px',boxSizing:'border-box'}}/>{offerForm.type==='offer' && <label style={{display:'flex',gap:'8px',background:offerForm.isDriver?'#e6f9ed':'#fff8e1',padding:'10px',borderRadius:'10px',border:`2px solid ${offerForm.isDriver?'#2ECC71':'#FFD60A'}`,fontSize:'11px',boxSizing:'border-box'}}><input type="checkbox" checked={offerForm.isDriver} onChange={e=>setOfferForm({...offerForm,isDriver:e.target.checked})}/>{t.declare}</label>}<button onClick={publishRide} disabled={!canPublish} style={{background:canPublish?'#2ECC71':'#ccc',padding:'14px',borderRadius:'10px',fontWeight:'bold',border:'none',boxSizing:'border-box'}}>{canPublish? (offerForm.type==='request'?`${t.publishSeeks} ${offerForm.from}→${offerForm.to}`:`${t.publish} ${offerForm.from}→${offerForm.to}`):`${t.fillFromTo} *`}</button></div>)}
        {tab==='my' && <div style={{padding:'10px',boxSizing:'border-box'}}>{rides.filter(r=>r.driverId===currentUser?.id).map((r:any)=><div key={r.id} style={{border:'2px solid #0F4C75',borderRadius:'12px',padding:'12px',marginBottom:'8px',boxSizing:'border-box'}}><b>{r.from} → {r.to}</b> <span style={{background:r.type==='request'?'#FFD60A':'#2ECC71',padding:'2px 6px',borderRadius:'6px',fontSize:'10px'}}>{r.type==='request'?t.youSeek:t.youOffer}</span><div style={{display:'flex',gap:'6px',marginTop:'8px'}}><button onClick={()=>{setOfferForm({type:r.type,from:r.from,to:r.to,fromCountry:r.fromCountry,toCountry:r.toCountry,time:r.time,returnTime:'',date:r.date,seats:r.seats.toString(),message:r.message,isDriver:r.isDriverVerified,carBrand:'',carColor:'',carReg:''}); setEditingRide(r.id); setTab('offer');}} style={{flex:1,background:'#0F4C75',color:'white',border:'none',padding:'8px',borderRadius:'8px',fontSize:'12px'}}>{t.edit}</button><button onClick={()=>deleteRide(r.id)} style={{padding:'8px 12px',borderRadius:'8px',border:'1px solid #ddd',background:'white'}}>🗑️</button></div></div>)}<button onClick={handleDeleteAccount} style={{width:'100%',marginTop:'20px',background:'black',color:'white',padding:'14px',borderRadius:'10px',fontWeight:'bold',border:'none'}}>{t.deleteAcc} ЗАВИНАГИ</button></div>}
      </div>
      <div style={{flexShrink:0,minHeight:'88px',background:'white',borderTop:'1px solid #e5e7eb',display:'flex',alignItems:'center',justifyContent:'space-between',padding:'10px 12px',gap:'8px',boxSizing:'border-box'}}>
