...
// АВТО-ПРЕВОДАЧ ЗА ТЕКСТА В ОБЯВАТА
const translateMessage = (msg:string, lang:Lang) => {
  ...
  const dict: Record<string,string> = {
    'Паметника': 'Monument',
    'паметника': 'monument',
    'жп спирка': 'train station',
    'ЖП спирка': 'Train station',
    'жп': 'train st.',
    'център Бяла': 'Byala center',
    'център': 'center',
    'Бяла': 'Byala',
    'пазара': 'market',
    'Полски Тръмбеш': 'Polski Trambesh',
    'Полско Косово': 'Polsko Kosovo',
    'Минавам през': 'Passing through',
    'Минавам': 'Passing',
    'минавам през': 'passing through',
    'в': 'at',
    '-': '-'
  };
  Object.keys(dict).forEach(bg => {
    translated = translated.split(bg).join(dict[bg]);
  });
  return translated;
};
...
export default function Home() {
  ...
  // ТУК Е ФИКСА ЗА ШРИФТА - вече няма да е Моuме
