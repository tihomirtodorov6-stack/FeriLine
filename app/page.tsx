...
export default function Home(){
  ...
  // ===== НОВАТА ЧАСТ - ДОБАВЕНА БЕЗ ДА МАХАМЕ НИЩО ОТ ОРИГИНАЛА =====
  ...
  const addShop = async ()=>{
    if(!shopName ||!ownerPhoneInput){ alert('Име и телефон на собственика!'); return; }
      ...
      <div style={{flex:1,overflowY:'auto',background:'#f9fafb'}} ref={ridesContainerRef}>
        ...
        {tab==='shops' && isAdmin && (
          <div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'12px'}}>
            <div style={{background:'#22c55e',color:'white',padding:'12px',borderRadius:'12px'}}><b>👑 Админ Магазини - само +447935463970</b><div style={{fontSize:'11px'}}>Тук добавяш магазин по телефон. 1 е готово.</div></div>
            ...
            {shops.map((s:any)=><div key={s.id} style={{background:'white',padding:'10px',borderRadius:'8px',border:'1px solid #eee'}}><b>{s.name}</b> - {s.phone} - {s.delivery_fee}лв при магазина</div>)}
          </div>
        )}
        ...
        {tab==='my' && <div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'10px'}}>
          {/* 2. МАГАЗИНЕР - ДОБАВЯ ЛИСТ СЪС СНИМКИ - ВЪТРЕ В ПРИЛОЖЕНИЕТО - ДОБАВЕНО */}
          ...
          {/* 3. ШОФЬОР - ПОЛУЧАВА ПОРЪЧКИ - ВЪТРЕ В ПРИЛОЖЕНИЕТО - ДОБАВЕНО */}
          ...
          {/* ОРИГИНАЛНОТО ОТ ТВОЯ ФАЙЛ - НЕ Е ПИПАНО */}
