...
const TRANSLATIONS = {
  bg: {
    siteFree:'Сайтът е безплатен', sharedCosts:'Пътуването е споделени разходи', exit:'Изход', find:'Намери', my:'Моите', offer:'Предложи', admin:'👑 Админ',
    important:'ℹ️ ВАЖНО - 2 неща:', important1:'1. Сайтът VoziMe е 100% безплатен.', important1b:'Ние не вземаме комисионна.', important2:'2. Самото пътуване НЕ е безплатно.', important2b:'Шофьор и пътник се договарят ЛИЧНО.',
    all:'Всички', drivers:'Шофьори', passengers:'Пътници', search:'🔍 Лондон, София...', notFree:'⚠️ Пътуването НЕ е безплатно - лична договорка.', connectDirect:'📞 СВЪРЖИ СЕ ДИРЕКТНО:', call:'📞 Обади се', offers:'🚗 ПРЕДЛАГА', seeks:'🙋 ТЪРСИ',
    imDriver:'🚗 Аз съм Шофьор', imPassenger:'🙋 Аз съм Пътник', from:'От', to:'До', brand:'Марка', color:'Цвят', reg:'Рег. номер', seatsNeed:'Места нужни', seatsFree:'Свободни места', note:'Бележка...', declare:'ДЕКЛАРИРАМ: Пътувам лично, само споделени разходи, БЕЗ печалба.', fillFromTo:'Попълни От и До', publishSeeks:'🙋 Публикувай че ТЪРСИШ', publish:'🚗 Публикувай', edit:'Редактирай', youSeek:'ТЪРСИШ', youOffer:'ПРЕДЛАГАШ',
    footerTitle:'❤️ VoziMe е безплатна', footerSub:'Сайтът не взема комисионна.', footerWarn:'Пътуванията НЕ са безплатни - лична договорка.', enableLoc:'📍 Включи локация', report:'🚩 Докладвай', banned:'⛔ БАННАТ СИ', maintenanceTitle:'🔧 Профилактика', maintenanceMsg:'Работим по подобрения.',
    deleteAcc:'🗑️ Изтрий акаунт', tabLogin:'Вход', tabReg:'Регистрация', loginSub:'Влез за да продължиш', loginBtn:'Влез →', regBtn:'Регистрирай се →', firstName:'Име', lastName:'Фамилия', phone:'Телефон +447...', time:'Час на тръгване',
    needVerify:'⚠️ Трябва верификация за да пускаш обяви!', needVerifyLong:'Отиди в Моите -> Стани Проверен със снимка. Без това не можеш да пускаш обяви - за сигурност.',
    verifyTitle:'✅ Стани Проверен - Безплатно', verifyDesc:'Направи снимка как държиш лист с текст: VoziMe + днешна дата',
    writeOnPaper:'На листа напиши:', step1:'1. VoziMe', step2:'2. Днешна дата: ', step3:'3. Твоето име',
    deleteRide:'🗑️ Трий', deleteAccKeep:'🗑️ Изтрий акаунт (пази се 6м)',
    photoSent:'Снимката е изпратена! Админ ще я одобри до 24ч.', pendingApproval:'⏳ Чака одобрение от админ. Снимката е изпратена.',
    uploading:'Качва...', sendForCheck:'📸 Изпрати за проверка', verifiedOn:'Верифициран на', noRides:'Нямаш обяви', noRidesVerify:'- първо се верифицирай',
    chooseFile:'Избери файл', noFile:'няма избран файл', seatsPlace:'места', agreementConsent:'Регистрирайки се се съгласяваш снимката за верификация да се пази 1г за сигурност.'
  },
  en: {
    siteFree:'Site is free', sharedCosts:'Travel is shared costs', exit:'Exit', find:'Find', my:'My rides', offer:'Offer', admin:'👑 Admin',
    important:'ℹ️ IMPORTANT - 2 things:', important1:'1. VoziMe is 100% free.', important1b:'We take no commission.', important2:'2. Trip is NOT free.', important2b:'Driver and passenger agree personally.',
    all:'All', drivers:'Drivers', passengers:'Passengers', search:'🔍 London, Sofia...', notFree:'⚠️ Trip NOT free - personal agreement.', connectDirect:'📞 CONNECT DIRECTLY:', call:'📞 Call', offers:'🚗 OFFERS', seeks:'🙋 SEEKS',
    imDriver:'🚗 I am Driver', imPassenger:'🙋 I am Passenger', from:'From', to:'To', brand:'Brand', color:'Color', reg:'Reg number', seatsNeed:'Seats needed', seatsFree:'Free seats', note:'Note...', declare:'I DECLARE: Personal travel, shared costs only, NO profit.', fillFromTo:'Fill From and To', publishSeeks:'🙋 Publish SEEK', publish:'🚗 Publish', edit:'Edit', youSeek:'YOU SEEK', youOffer:'YOU OFFER',
    footerTitle:'❤️ VoziMe is free', footerSub:'No commission.', footerWarn:'Trips NOT free.', enableLoc:'📍 Enable location', report:'🚩 Report', banned:'⛔ BANNED', maintenanceTitle:'🔧 Maintenance', maintenanceMsg:'Working on improvements.',
    deleteAcc:'🗑️ Delete account', tabLogin:'Login', tabReg:'Register', loginSub:'Login to continue', loginBtn:'Login →', regBtn:'Register →', firstName:'First name', lastName:'Last name', phone:'Phone +447...', time:'Departure time',
    needVerify:'⚠️ Verification needed to post!', needVerifyLong:'Go to My rides -> Get Verified with photo. You cannot post without it - for safety.',
    verifyTitle:'✅ Get Verified - Free', verifyDesc:'Take photo holding paper: VoziMe + today date',
    writeOnPaper:'On paper write:', step1:'1. VoziMe', step2:'2. Today date: ', step3:'3. Your name',
    deleteRide:'🗑️ Delete', deleteAccKeep:'🗑️ Delete account (kept 6m)',
    photoSent:'Photo sent! Admin will approve within 24h.', pendingApproval:'⏳ Waiting for admin approval. Photo sent.',
    uploading:'Uploading...', sendForCheck:'📸 Send for check', verifiedOn:'Verified on', noRides:'No rides', noRidesVerify:'- verify first',
    chooseFile:'Choose File', noFile:'no file selected', seatsPlace:'seats', agreementConsent:'By registering you agree your verification photo is kept 1 year for safety.'
  }
};
...
export default function Home(){
  ...
  return (
    <main style={{height:'100dvh',width:'100%',maxWidth:'480px',margin:'0 auto',background:'white',display:'flex',flexDirection:'column',overflow:'hidden',fontFamily:'-apple-system, sans-serif'}}>
      ...
      {!isVerified && tab!=='my' && <div style={{background:'#FFD60A',color:'black',padding:'8px',textAlign:'center',fontSize:'11px',fontWeight:'bold',flexShrink:0}}>{t.needVerify}</div>}
      ...
      <div style={{flex:1,overflowY:'auto',background:'#f9fafb'}} ref={ridesContainerRef}>
        ...
        {tab==='admin' && isAdmin && (<div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'12px'}}>
          <div style={{background:'#0F4C75',color:'white',padding:'12px',borderRadius:'12px'}}><div style={{fontWeight:'bold',fontSize:'14px'}}>👑 ADMIN • Verification + 6 months archive</div><div style={{fontSize:'11px',opacity:0.8}}>All users kept even if deleted</div></div>

          <div style={{background:'white',border:'2px solid #2ECC71',borderRadius:'12px',padding:'12px'}}>
            <div style={{fontWeight:'bold',fontSize:'13px'}}>👥 ALL USERS - {filteredUsers.length} (sorted by country + name)</div>
            <input placeholder={lang==='bg'?'Търси по телефон или име...':'Search by phone or name...'} value={userSearch} onChange={e=>setUserSearch(e.target.value)} style={{width:'100%',marginTop:'8px',padding:'10px',borderRadius:'8px',border:'1px solid #ddd',fontSize:'12px'}}/>
                ...
                return (
                  <div key={u.id} style={{background:isDeleted?'#ffeaea':u.is_verified?'#e6f9ed':'#fffbe6',border:`1px solid ${isDeleted?'#FF3B30':u.is_verified?'#2ECC71':'#FFD60A'}`,padding:'8px',borderRadius:'8px'}}>
                    <div style={{display:'flex',justifyContent:'space-between'}}>
                      <div style={{fontWeight:'bold',fontSize:'12px'}}>{getFlag(country)} {country} • {u.first_name} {u.last_name} {u.is_verified?'✅':''} {isDeleted&&`🔴 DELETED ${deletedAgo}d ago`}</div>
                      <div style={{fontSize:'10px'}}>{u.phone}</div>
                    </div>
                    <div style={{fontSize:'10px',marginTop:'2px'}}>ID: {u.id.slice(0,8)}... • {u.clean_phone} • Reg: {new Date(u.created_at||Date.now()).toLocaleDateString()} {isDeleted && `• Deleted: ${new Date(Number(u.deleted_at)).toLocaleString()}`}</div>
                    {u.verification_photo_url && <div style={{marginTop:'4px',display:'flex',gap:'6px'}}><a href={u.verification_photo_url} target="_blank" style={{fontSize:'10px',background:'#0F4C75',color:'white',padding:'4px 8px',borderRadius:'6px',textDecoration:'none'}}>📸 View verification photo</a>{!u.is_verified && <button onClick={()=>approveUser(u.id)} style={{fontSize:'10px',background:'#2ECC71',border:'none',padding:'4px 8px',borderRadius:'6px',fontWeight:'bold'}}>✅ Approve</button>}</div>}
                  </div>
                )
              })}
            </div>
          ...
          <div style={{background:'white',border:'2px solid #FF3B30',borderRadius:'12px',padding:'12px'}}><div style={{fontWeight:'bold',fontSize:'13px',color:'#FF3B30'}}>🔧 STOP SITE</div><div style={{display:'flex',gap:'8px',marginTop:'10px',alignItems:'center'}}><button onClick={toggleMaintenance} style={{background:maintenance.enabled?'#2ECC71':'#FF3B30',color:'white',border:'none',padding:'10px 16px',borderRadius:'8px',fontWeight:'bold',fontSize:'12px'}}>{maintenance.enabled?'✅ STOPPED - START':'🔴 STOP SITE NOW'}</button><span style={{fontSize:'11px',fontWeight:'bold',color:maintenance.enabled?'#FF3B30':'#2ECC71'}}>{maintenance.enabled?'🔴 STOPPED':'🟢 Running'}</span></div><input placeholder={lang==='bg'?'Съобщение...':'Message...'} value={maintenance.msg} onChange={e=>updateMaintenanceMsg(e.target.value)} style={{width:'100%',marginTop:'8px',padding:'8px',borderRadius:'8px',border:'1px solid #ddd',fontSize:'11px'}}/></div>
        ...
        {tab==='my' && <div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'10px'}}>
          <div style={{background:isVerified?'#e6f9ed':'#fff3cd',border:`2px solid ${isVerified?'#2ECC71':'#FFD60A'}`,borderRadius:'12px',padding:'12px'}}>
            <div style={{fontWeight:'bold',fontSize:'14px'}}>{isVerified? (lang==='bg'?'✅ Верифициран си! Можеш да пускаш обяви.':'✅ Verified! You can post rides.') : t.verifyTitle}</div>
            <div style={{fontSize:'11px',marginTop:'4px'}}>{isVerified?`${t.verifiedOn} ${currentUser.verified_at? new Date(currentUser.verified_at).toLocaleDateString(lang==='bg'?'bg-BG':'en-GB'):''}`:t.verifyDesc}</div>
            {!isVerified && <div style={{marginTop:'10px'}}>
              <div style={{background:'white',padding:'8px',borderRadius:'8px',fontSize:'11px',border:'1px solid #ddd',marginBottom:'8px'}}>
                {t.writeOnPaper}<br/>{t.step1}<br/>{t.step2}{new Date().toLocaleDateString(lang==='bg'?'bg-BG':'en-GB')}<br/>{t.step3}
              </div>
              <input type="file" accept="image/*" capture="user" onChange={e=>setVerifyFile(e.target.files?.[0]||null)} style={{width:'100%',fontSize:'12px'}}/>
              {verifyFile && <div style={{fontSize:'11px',marginTop:'4px'}}>{verifyFile.name}</div>}
              {verifyFile && <button onClick={handleVerificationUpload} disabled={uploadingVerify} style={{width:'100%',marginTop:'8px',background:'#0F4C75',color:'white',padding:'12px',borderRadius:'8px',fontWeight:'bold',border:'none'}}>{uploadingVerify?t.uploading:t.sendForCheck}</button>}
              {currentUser.verification_photo_url &&!isVerified && <div style={{fontSize:'11px',marginTop:'6px',color:'#b45309'}}>{t.pendingApproval}</div>}
            </div>}
          </div>
          ...
          {rides.filter(r=> r.driverId===currentUser?.id || r.driverPhone===currentUser?.phone || clean(r.driverPhone)===clean(currentUser?.phone)).length===0 && <div style={{textAlign:'center',padding:'10px',color:'#666',fontSize:'12px'}}>{t.noRides} {isVerified?'':t.noRidesVerify}</div>}
