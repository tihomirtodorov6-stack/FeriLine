...
export default function Home(){
  ...
  // ВЗЕМА ТЕКУЩАТА ЛОКАЦИЯ - ВИНАГИ ПРИ ВЛИЗАНЕ, БЕЗ ЗАПАМЕТЯВАНЕ
  const fetchLocation = ()=>{
    ...
    navigator.geolocation.getCurrentPosition(async (pos)=>{
      try{
        ...
        // ако не познае областта от display_name, пробва от state
        ...
        setMyLocation(loc);
        // автоматично слага филтъра за областта
  ...
  useEffect(()=>{
    ...
    fetchLocation(); // при първо влизане

    // когато аппа излезе от background и се отвори пак - взема НОВА текуща локация
    const onVis = ()=>{ if(document.visibilityState==='visible'){ fetchLocation(); } };
        ...
        {tab==='find' && <div style={{padding:'12px',display:'flex',flexDirection:'column',gap:'12px', paddingBottom:'100px'}}>
          <div style={{background:'#f8f9fa',padding:'12px',borderRadius:'16px',border:'1px solid #eee',display:'flex',flexDirection:'column',gap:'8px'}}>
            <div style={{display:'flex',gap:'6px',alignItems:'center'}}>
              ...
              <button onClick={fetchLocation} style={{padding:'10px 12px',borderRadius:'10px',border:'none',fontWeight:'bold',fontSize:'11px',background:'#2ECC71',color:'#0F4C75'}}>📍 {locLoading? '...' : 'Обнови'}</button>
            ...
            <div style={{fontSize:'10px',color:'#666'}}>Текуща локация: {myLocation? `${myLocation.grad||''} ${myLocation.obshtina} ${myLocation.oblast}` : 'непозната'} • Показва {visibleRides.length} от {visibleRidesBase.length}</div>
          </div>
        ...
        {tab==='offer' && (
          <div style={{padding:'16px',display:'flex',flexDirection:'column',gap:'12px', paddingBottom:'100px'}}>
            ...
            <button onClick={publishRide} disabled={!canPublish} style={{background:canPublish?'#2ECC71':'#ccc',color:canPublish?'#0F4C75':'#888',padding:'16px',borderRadius:'12px',fontWeight:'bold',border:'none',fontSize:'16px'}}>
              {editingRide? '💾 Запази промените' : (canPublish? 'Публикувай 🚗' : 'Попълни Област/Община/Град, От, До, кола *')}
            </button>
          </div>
        )}
        {tab==='admin' && isAdmin && <div style={{padding:'12px', paddingBottom:'100px'}}>Админ - {reports.length} сигнала<br/>Общо обяви: {rides.length} • Текуща локация: {myLocation? `${myLocation.grad} ${myLocation.obshtina} ${myLocation.oblast}` : 'непозната'}</div>}
      </div>
      <Footer/>
    </main>
  );
