...
export default function Home(){
  ...
  // === ОРИГИНАЛНИ STATE-ОВЕ ОТ ТВОЯ КОД - НЕ ПИПАМ ===
  ...
  // === НОВИ ДОПЪЛНЕНИЯ - ЗА ДА РАБОТИ АДМИН И МАГАЗИНЕР С ЦЕНИ ===
  ...
  // === ОРИГИНАЛЕН loadShops ОТ ТВОЯ КОД + ДОПЪЛНЕН С shop_profiles ЗА ДА РАБОТИ РЕДАКЦИЯТА ===
  ...
  // === ОПРАВЕН toggleShopActive - С ERROR HANDLING ЗА ДА ТРЪГНЕ СПРИ БУТОНА ===
  const toggleShopActive = async (shop:any)=>{
    ...
    if(error){ alert('Грешка СПРИ бутон: '+error.message+'\nПусни FIX_RLS_shops.sql в Supabase SQL Editor!'); return; }
    await loadShops();
  };
  // === НОВИ ФУНКЦИИ ЗА АДМИН - РЕДАКЦИЯ НА МАГАЗИН, СМЯНА НОМЕР, ШОФЬОРИ ===
  ...
  // === ОПРАВЕНА ФУНКЦИЯ ЗА ЦЕНА - приема 6,25 и 6.25€ - ОТ ТВОЯ КОД + ДОПЪЛНЕНА С РЕДАКЦИЯ ===
  const addProductWithImage = async ()=>{
    ...
    alert(editingProduct?'Цената променена!':'Продукт добавен! Вижда се в Пазар веднага.');
  };
      ...
      <div style={{flex:1,overflowY:'auto',background:'#f9fafb'}} ref={ridesContainerRef}>
        ...
        {tab==='shops' && isAdmin && (
          <div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'12px'}}>
            <div style={{background:'#22c55e',color:'white',padding:'12px',borderRadius:'12px'}}><b>👑 Админ Магазини - ПЪЛЕН КОНТРОЛ - само +447935463970</b><div style={{fontSize:'11px'}}>СПРИ бутон спира достъпа ако не плати. Смяна номер, добавяне/премахване шофьор.</div></div>
              ...
              return (
                <div key={s.id} style={{background:'white',padding:'12px',borderRadius:'12px',border:s.vip_active===false?'3px solid red':'2px solid #22c55e'}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
                    <div><b style={{fontSize:'14px'}}>{s.name}</b><div style={{fontSize:'11px',color:'#666'}}>Тел: {s.phone} • {s.vip_active===false?'🔴 СПРЯН':'🟢 Активен'} • {s.delivery_fee}€ доставка</div></div>
                    <div style={{display:'flex',gap:'4px'}}><button onClick={()=>toggleShopActive(s)} style={{background:s.vip_active===false?'#22c55e':'#FF3B30',color:'white',border:'none',padding:'8px 12px',borderRadius:'8px',fontSize:'11px',fontWeight:'bold'}}>{s.vip_active===false?'✅ ПУСНИ':'⛔ СПРИ'}</button><button onClick={()=>deleteShop(s.id)} style={{background:'black',color:'white',border:'none',padding:'8px 10px',borderRadius:'8px',fontSize:'10px'}}>🗑️</button></div>
                  </div>
                  <div style={{marginTop:'10px',background:'#f9fafb',padding:'10px',borderRadius:'8px',border:'1px solid #e5e7eb'}}>
                    <div style={{fontWeight:'bold',fontSize:'11px'}}>✏️ Редактирай - ако си смени номера:</div>
                    ...
                    <button onClick={()=>updateShopInfo(s)} style={{width:'100%',marginTop:'6px',padding:'8px',background:'#0F4C75',color:'white',border:'none',borderRadius:'6px',fontSize:'11px',fontWeight:'bold'}}>💾 Запази - име, такса, НОВ ТЕЛЕФОН собственик</button>
                  </div>
                  <div style={{marginTop:'10px'}}>
                    <div style={{fontWeight:'bold',fontSize:'11px'}}>👥 Шофьори към този магазин:</div>
                    {owner && <div style={{fontSize:'11px',background:'#e6f9ed',padding:'6px',borderRadius:'6px',marginTop:'4px'}}>👑 Собственик: <b>{owner.phone}</b></div>}
                    ...
                    <div style={{display:'flex',gap:'6px',marginTop:'8px'}}>
                      <input value={edit.newDriverPhone} onChange={e=>setEditShopData({...editShopData, [s.id]: {...edit, newDriverPhone:e.target.value}})} placeholder="Нов шофьор: 0888..." style={{flex:1,padding:'8px',borderRadius:'6px',border:'1px solid #ddd',fontSize:'11px'}}/>
          ...
          {myShopProfile && myShopProfile.role==='shop_owner' && (
            <div style={{background:'white',border:'3px solid #22c55e',borderRadius:'14px',padding:'14px'}}>
              <div style={{background:'#22c55e',color:'white',padding:'10px',borderRadius:'10px',margin:'-14px -14px 12px -14px'}}>
                <div style={{fontWeight:'800',fontSize:'15px'}}>🛒 Моят магазин - {marketShops.find(s=>s.id===myShopProfile.shop_id)?.name||'Магазин'} - ти си собственик</div>
              ...
              <div style={{background:editingProduct?'#fef3c7':'#f0fdf4',border:`2px solid ${editingProduct?'#f59e0b':'#22c55e'}`,borderRadius:'12px',padding:'12px'}}>
                ...
                <div style={{marginTop:'8px',border:'1px dashed #22c55e',padding:'8px',borderRadius:'8px',background:'white'}}>
                  ...
                  {editingProduct?.image_url && !newProdFile && <div style={{fontSize:'10px',marginTop:'4px'}}><img src={editingProduct.image_url} style={{width:'50px',height:'50px',objectFit:'cover',borderRadius:'6px'}}/> Сегашна снимка</div>}
