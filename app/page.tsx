...
// ТВОЯ ОРИГИНАЛ + добавени shops и market за да не чупи t.shops
const TRANSLATIONS = {
...
// ТВОЯ ОРИГИНАЛ 100% - НЕ СЪМ ПИПАЛ НИЩО
...
export default function Home(){
  ...
  // ДОБАВЕНО: shops и market към таба - оригинала ти е find/my/offer/admin, добавям 2 нови без да махам старите
  ...
  // === НОВО: Магазини - добавено без да махам твоите states ===
  ...
  // === НОВО: Зареждане магазини - добавено, не махам твоите load-ове ===
  ...
  const toggleShopActive = async (shop:any)=>{
    ...
    if(error){ alert('Грешка СПРИ: '+error.message); return; }
    await loadShops();
  };
  ...
  // ТВОЯ ОРИГИНАЛ 100% - НЕ СЪМ ПИПАЛ НИЩО, САМО ДОБАВИХ loadShops
  ...
  return (
    <main style={{height:'100dvh',width:'100%',maxWidth:'480px',margin:'0 auto',background:'white',display:'flex',flexDirection:'column',overflow:'hidden',fontFamily:'-apple-system, sans-serif'}}>
      <div style={{flexShrink:0}}>
        ...
        {/* ТВОЯ ОРИГИНАЛ + 2 НОВИ БУТОНА - НЕ МАХАМ НИЩО */}
      ...
      <div style={{flex:1,overflowY:'auto',background:'#f9fafb'}} ref={ridesContainerRef}>
        ...
        {/* ТВОЯ ОРИГИНАЛ ADMIN 100% - НЕ СЪМ ПИПАЛ НИЩО */}
        ...
        {/* === НОВО: Магазини таб - ПЪЛЕН АДМИН КОНТРОЛ - не махам твоя admin таб, това е отделен таб === */}
        {tab==='shops' && isAdmin && (
          <div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'12px'}}>
            <div style={{background:'#22c55e',color:'white',padding:'12px',borderRadius:'12px'}}><b>👑 Админ Магазини - ПЪЛЕН КОНТРОЛ</b><div style={{fontSize:'11px'}}>Тук редактираш всичко - име, такса, телефон собственик (ако си смени номера), шофьори, СПРИ бутон.</div></div>
              ...
              return (
                <div key={s.id} style={{background:'white',padding:'12px',borderRadius:'12px',border:s.vip_active===false?'3px solid red':'2px solid #22c55e'}}>
                  ...
                  <div style={{marginTop:'10px',background:'#f9fafb',padding:'10px',borderRadius:'8px',border:'1px solid #e5e7eb'}}>
                    <div style={{fontWeight:'bold',fontSize:'11px'}}>✏️ Редактирай - име, такса, ТЕЛЕФОН собственик (ако си смени номера):</div>
                    ...
                    <button onClick={()=>updateShopInfo(s)} style={{width:'100%',marginTop:'6px',padding:'10px',background:'#0F4C75',color:'white',border:'none',borderRadius:'6px',fontSize:'11px',fontWeight:'bold'}}>💾 Запази - име, такса, НОВ ТЕЛ собственик</button>
                  </div>
          ...
          <div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'10px'}}>
            {!selectedMarketShop? (
              <>
                ...
                {marketShops.map((s:any)=><div key={s.id} onClick={()=>loadMarketProducts(s.id)} style={{background:'white',border:'2px solid #22c55e',borderRadius:'12px',padding:'12px',cursor:'pointer'}}><b>{s.name}</b> - {s.city}<br/><span style={{fontSize:'11px'}}>Доставка {s.delivery_fee}€</span><div style={{fontSize:'11px',color:'#22c55e',fontWeight:'bold'}}>Виж продукти →</div></div>)}
              </>
            ) : (
              <>
        ...
        {/* ТВОЯ ОРИГИНАЛ MY 100% + ДОБАВЕН МАГАЗИН ОТГОРЕ - НЕ МАХАМ НИЩО */}
        {tab==='my' && <div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'10px'}}>
          {/* НОВО: Магазин за собственика - добавено отгоре, твоя оригинал отдолу е непокътнат */}
          {myShopProfile && (
