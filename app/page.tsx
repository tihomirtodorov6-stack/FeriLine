...
export default function Home(){
  ...
  useEffect(()=>{ const savedLang = localStorage.getItem('vozime_lang') as 'bg'|'en'; if(savedLang) setLang(savedLang); const cu=localStorage.getItem('vozime_current'); if(cu) setCurrentUser(JSON.parse(cu)); syncCurrentUser(); loadRides(); loadMaintenance(); loadBans(); loadReports(); fetchLocation(); const interval = setInterval(()=>{ loadMaintenance(); loadBans(); loadReports(); loadRides(); syncCurrentUser(); if(isAdminPhone(JSON.parse(localStorage.getItem('vozime_current')||'{}').phone||'')) loadAllUsers(); }, 5000); return ()=> clearInterval(interval); },[]);
  ...
  const isVerified = currentUser?.is_verified || isAdmin; // админа може винаги, ти си админ
  ...
  return (
    <main style={{height:'100dvh',width:'100%',maxWidth:'480px',margin:'0 auto',background:'white',display:'flex',flexDirection:'column',overflow:'hidden',fontFamily:'-apple-system, sans-serif'}}>
      ...
      {!isVerified && <div style={{background:'#FFD60A',color:'black',padding:'8px',textAlign:'center',fontSize:'11px',fontWeight:'bold',flexShrink:0}}>{t.needVerify} - {t.refresh}</div>}
      ...
      <div style={{flex:1,overflowY:'auto',background:'#f9fafb'}} ref={ridesContainerRef}>
        ...
        {tab==='admin' && isAdmin && (<div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'12px'}}>
          <div style={{background:'#0F4C75',color:'white',padding:'12px',borderRadius:'12px'}}><div style={{fontWeight:'bold',fontSize:'14px'}}>👑 ADMIN • Verification + 6 months archive</div><div style={{fontSize:'11px',opacity:0.8}}>All users kept even if deleted</div><button onClick={syncCurrentUser} style={{marginTop:'8px',background:'#2ECC71',border:'none',padding:'6px 12px',borderRadius:'8px',fontSize:'11px',fontWeight:'bold'}}>{t.refresh}</button></div>
              ...
              {filteredUsers.map(u=>{
                ...
                return (
                  <div key={u.id} style={{background:isDeleted?'#ffeaea':u.is_verified?'#e6f9ed':'#fffbe6',border:`1px solid ${isDeleted?'#FF3B30':u.is_verified?'#2ECC71':'#FFD60A'}`,padding:'8px',borderRadius:'8px'}}>
                    <div style={{display:'flex',justifyContent:'space-between'}}><div style={{fontWeight:'bold',fontSize:'12px'}}>{getFlag(country)} {country} • {u.first_name} {u.last_name} {u.is_verified?'✅':''} {isDeleted&&`🔴 DELETED ${deletedAgo}d ago`}</div><div style={{fontSize:'10px'}}>{u.phone}</div></div>
        ...
        {tab==='my' && <div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'10px'}}>
          <div style={{background:isVerified?'#e6f9ed':'#fff3cd',border:`2px solid ${isVerified?'#2ECC71':'#FFD60A'}`,borderRadius:'12px',padding:'12px'}}>
            ...
            {!isVerified && <div style={{marginTop:'10px'}}>
              <div style={{background:'white',padding:'8px',borderRadius:'8px',fontSize:'11px',border:'1px solid #ddd',marginBottom:'8px'}}>{t.writeOnPaper}<br/>{t.step1}<br/>{t.step2}{new Date().toLocaleDateString(lang==='bg'?'bg-BG':'en-GB')}<br/>{t.step3}</div>
              ...
              {currentUser.verification_photo_url &&!currentUser.is_verified && <div style={{fontSize:'11px',marginTop:'6px',color:'#b45309'}}>{t.pendingApproval}</div>}
            </div>}
            {isVerified && <button onClick={syncCurrentUser} style={{marginTop:'8px',background:'#0F4C75',color:'white',border:'none',padding:'8px 12px',borderRadius:'8px',fontSize:'11px'}}>{t.refresh}</button>}
          </div>
