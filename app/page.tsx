...
const t = {
  BG: {
    ...
    donateTitle:'❤️ VoziMe.bg е безплатен за всички',
    donateText:'Ако сайтът ти е бил полезен, можеш да подкрепиш поддръжката му с дарение по желание от доволни потребители.',
    donateButton:'☕ Подкрепи сайта в Ko-fi',
    donateNote:'Дарението е за сайта, не за шофьорите. Не е задължително и не дава предимство.'
  },
  EN: {
    ...
    donateTitle:'❤️ VoziMe.bg is free for everyone',
    donateText:'If this site was useful, you can support it with a voluntary donation from happy users.',
    donateButton:'☕ Support on Ko-fi',
    donateNote:'Donation is for the site, not drivers. Not required and gives no advantage.'
  }
};
...
const translateMessage = (msg:string, lang:Lang) => {
  ...
  const dict: Record<string,string> = {
    'Паметника': 'Monument', 'паметника': 'monument',
    'жп спирка': 'train station', 'ЖП спирка': 'Train station', 'жп': 'train st.',
    'Полски Тръмбеш': 'Polski Trambesh', 'пазара': 'market', 'пазара)': 'market)',
    'Минавам през': 'Passing through', 'минавам през': 'passing through'
  };
...
export default function Home() {
  ...
  useEffect(()=>{
    ...
    if(savedLang) {
      setLang(savedLang);
      if(savedLang === 'EN') setOfferForm(f=>({...f, from:'Polsko Kosovo', to:'Byala', date:'Today'}));
    }
  ...
  const changeLang = (l:Lang)=>{
    setLang(l); localStorage.setItem('vozime_lang', l);
    if(l === 'EN'){
  ...
  const publishRide = ()=>{
    ...
    setOfferForm({from: lang==='EN'?'Polsko Kosovo':'Полско Косово', to: lang==='EN'?'Byala':'Бяла', time:'09:30', returnTime:'12:30', date: lang==='EN'?'Today':'Днес', seats:'4', message:''});
    setTab('my');
  };
      ...
      <div style={contentStyle}>
        {tab==='find' && (
          <div style={{padding:'12px',display:'flex',flexDirection:'column',gap:'12px'}}>
            {rides.filter(r=>!r.isFull).map(ride=>{
              ...
              const displayMessage = translateMessage(ride.message, lang);
              return (
              <div key={ride.id} style={{border:'1px solid #eee',borderRadius:'16px',padding:'14px'}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:'8px'}}>
                  <div style={{fontWeight:'bold',fontSize:'16px'}}>{displayFrom} → {displayTo} <span style={{fontSize:'11px',background:'#f1f3f4',padding:'3px 8px',borderRadius:'10px',marginLeft:'6px'}}>{displayDate}</span></div>
                ...
                <div style={{marginTop:'10px',padding:'10px',background:'#f8f9fa',borderRadius:'12px'}}>
                  ...
                  <div style={{fontSize:'13px',marginTop:'6px',lineHeight:'1.3'}}>"{displayMessage}"</div>
                </div>
        ...
        {tab==='offer' && (
          <div style={{padding:'16px',display:'flex',flexDirection:'column',gap:'10px'}}>
            ...
            <div style={{background:'#FFF3CD',padding:'10px',borderRadius:'10px',fontSize:'11px',color:'#856404',textAlign:'center',fontWeight:'bold'}}>{tr.shared} • {tr.callToArrange}</div>
            <DonateBlock />
          </div>
        )}
      </div>
