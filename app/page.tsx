...
export default function Home(){
  ...
  const ridesEndRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(()=>{
    if(ridesContainerRef.current){
      ridesContainerRef.current.scrollTop = ridesContainerRef.current.scrollHeight;
    }
  },[rides, filterType, filterText, tab]);
  ...
  return (
    <main style={{height:'100dvh',width:'100%',maxWidth:'480px',margin:'0 auto',background:'white',display:'flex',flexDirection:'column',overflow:'hidden',fontFamily:'-apple-system, sans-serif'}}>
      ...
      <div style={{flexShrink:0}}>
        ...
        <header style={{height:'56px',minHeight:'56px',background:'#0F4C75',color:'white',display:'flex',alignItems:'center',padding:'0 12px',gap:'8px'}}>
          ...
          <button onClick={logout} style={{fontSize:'11px',background:'#FF3B30',border:'none',color:'white',padding:'6px 12px',borderRadius:'12px',fontWeight:'bold'}}>Изход</button>
        </header>
        <div style={{height:'52px',display:'flex',gap:'6px',padding:'6px',background:'#f1f3f4'}}>
          <button onClick={()=>setTab('find')} style={{flex:1,borderRadius:'10px',border:'none',fontWeight:'bold',background:tab==='find'?'#0F4C75':'white',color:tab==='find'?'white':'#666',fontSize:'12px'}}>Намери ({filteredRides.length})</button>
      ...
      {/* FIXED INFO PANEL - НЕ СЕ СКРОЛВА */}
      {tab==='find' && (
        <div style={{flexShrink:0,background:'white',padding:'8px 10px',borderBottom:'1px solid #e5e7eb',display:'flex',flexDirection:'column',gap:'8px'}}>
          ...
          <div style={{display:'flex',gap:'6px'}}>
            ...
            <button onClick={()=>setFilterType('offer')} style={{flex:1,padding:'8px',borderRadius:'8px',border:'none',fontWeight:'bold',fontSize:'10px',background:filterType==='offer'?'#2ECC71':'white'}}>🚗 Шофьори ({rides.filter(r=>r.type==='offer').length})</button>
            <button onClick={()=>setFilterType('request')} style={{flex:1,padding:'8px',borderRadius:'8px',border:'none',fontWeight:'bold',fontSize:'10px',background:filterType==='request'?'#FFD60A':'white'}}>🙋 Пътници ({rides.filter(r=>r.type==='request').length})</button>
          </div>
          <input placeholder="🔍 Лондон, София..." value={filterText} onChange={e=>setFilterText(e.target.value)} style={{padding:'10px',borderRadius:'10px',border:'1px solid #ddd',fontSize:'13px',width:'100%'}}/>
        </div>
      )}

      {/* SCROLLABLE ONLY RIDES */}
      <div style={{flex:1,overflowY:'auto',background:'#f9fafb'}} ref={ridesContainerRef}>
        {tab==='find' && (
          <div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'10px'}}>
            ...
            <div ref={ridesEndRef} style={{height:'1px'}} />
          </div>
        )}
        {tab==='offer' && (
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',zIndex:50,display:'flex',alignItems:'center',justifyContent:'center',padding:'16px'}}>
          <div style={{background:'white',borderRadius:'16px',padding:'20px',maxWidth:'400px',maxHeight:'80vh',overflowY:'auto'}}>
            ...
            <div style={{fontSize:'12px',lineHeight:'1.5',display:'flex',flexDirection:'column',gap:'10px'}}>
              <div><b>1. Платформата е 100% безплатна.</b><br/>VoziMe не е превозвач, не взема комисионна. Ние сме борса за обяви.</div>
              <div><b>2. Пътуването НЕ е безплатно - лична договорка за разходите.</b><br/>Шофьор и пътник се свързват директно и се договарят ЛИЧНО какви са споделените разходи.</div>
