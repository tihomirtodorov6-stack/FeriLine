...
export default function Home(){
  ...
  // САМО ТУК Е ФИКСА - ЕКРАНА НЕ МЪРДА
  ...
  const footerStyle:React.CSSProperties={minHeight:'78px',flexShrink:0,background:'white',borderTop:'1px solid #e5e7eb',display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px',flexShrink:0};
  ...
  return (
    <main style={appStyle}>
      ...
      <div ref={scrollRef} style={contentStyle}>
        {tab==='find' && <div style={{padding:'12px',display:'flex',flexDirection:'column',gap:'12px', paddingBottom:'80px'}}>{visibleRides.map((r:any)=><div key={r.id} style={{border:'1px solid #eee',borderRadius:'16px',padding:'14px',background:'white'}}><div style={{display:'flex',justifyContent:'space-between'}}><b>{r.from} → {r.to}</b><span style={{background:'#FF3B30',color:'white',fontSize:'10px',padding:'3px 8px',borderRadius:'8px'}}>НЕ Е БЕЗПЛАТНО</span></div><div style={{fontSize:'12px',color:'#666',marginTop:'4px'}}>🕒 Тръгване: {r.time} • Очакв.: {r.returnTime} • {r.date}</div><div style={{fontSize:'12px',marginTop:'4px'}}>🚗 {r.carInfo} • 👥 {r.seats} свободни места</div><div style={{fontSize:'13px',marginTop:'8px'}}>👤 {r.driverName} ✓ ШОФЬОР</div></div>)}</div>}

        {tab==='my' && <div style={{padding:'12px',display:'flex',flexDirection:'column',gap:'12px', paddingBottom:'80px'}}>{myRides.map((r:any)=>(
        ...
        {tab==='offer' && (
          <div style={{padding:'16px',display:'flex',flexDirection:'column',gap:'12px', paddingBottom:'80px'}}>
        ...
        {tab==='admin' && isAdmin && <div style={{padding:'12px', paddingBottom:'80px'}}>Админ - {reports.length} сигнала</div>}
      </div>
      <Footer/>
    </main>
  );
