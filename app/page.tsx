...
const t={
  BG:{
    ...
    from:'От',to:'До',departure:'Тръгване',arrival:'Пристигане', seats:'Свободни места *', seatsDesc:'Колко пътници взимаш?', seatsPlaceholder:'Свободни места * - напр. 4',
    message:'Къде точно минаваш? През кои градове?',date:'Кога', today:'Днес',tomorrow:'Утре',
    publish:'Публикувай',save:'Запази', call:'Обади се',viber:'Viber',
    ...
    mustConfirm:'Попълни От, До и кола + декларация!',
    notFreeBadge:'НЕ Е БЕЗПЛАТНО',
    brand:'Марка *', color:'Цвят *', reg:'Рег. номер *',
    dateLabel:'Дата', departLabel:'Час тръгване', arrivalLabel:'Очакв. пристигане',
  },
  EN:{ appName:'VoziMe.bg', route:'Ride-sharing', networkBanner:'VoziMe.bg is part of', networkName:'dropoffpay.co.uk', login:'Login',register:'Register',logout:'Logout', find:'Find',my:'My',offer:'Offer',edit:'Edit', admin:'Admin', from:'From',to:'To',departure:'Depart',arrival:'Arrival', seats:'Free seats *', seatsDesc:'How many passengers?', seatsPlaceholder:'Free seats * - e.g. 4', message:'Where?',date:'When', today:'Today',tomorrow:'Tomorrow', publish:'Publish',save:'Save', call:'Call',viber:'Viber', noRides:'No rides', noMyRides:'No rides', callToArrange:'NOT free', donateTitle:'Platform free', donateSub:'Ride NOT free • Shared cost', donateButton:'Ko-fi', verified:'VERIFIED', offering:'Offering', searching:'Seeking', driverConfirm:'I am driver', mustConfirm:'Fill all!', notFreeBadge:'NOT FREE', brand:'Brand *', color:'Color *', reg:'Reg *', dateLabel:'Date', departLabel:'Depart', arrivalLabel:'Arrival', }
};
...
export default function Home(){
  ...
  const publishRide=()=>{
    if(!currentUser || !canPublish){alert(t.BG.mustConfirm);return;}
    const now = Date.now();
  ...
  const appStyle:React.CSSProperties={height:'100dvh',width:'100%',maxWidth:'480px',margin:'0 auto',background:'white',display:'flex',flexDirection:'column',overflow:'hidden',position:'relative',fontFamily:'-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif'};
  ...
  const footerStyle:React.CSSProperties={minHeight:'78px',flexShrink:0,background:'white',borderTop:'1px solid #e5e7eb',display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 12px calc(12px + env(safe-area-inset-bottom)) 12px',boxShadow:'0 -4px 20px rgba(0,0,0,0.06)',zIndex:20};
  ...
  const Footer=()=>(
    <div style={footerStyle}>
      <div style={{flex:1,minWidth:0}}>
        <div style={{fontSize:'14px',fontWeight:'800',color:'#0F4C75',display:'flex',alignItems:'center',gap:'6px'}}>❤️ Платформата е безплатна</div>
  ...
  return (
    <main style={appStyle}>
      <div style={{background:'#0F4C75',color:'white',padding:'8px',textAlign:'center',fontSize:'11px',flexShrink:0}}>VoziMe.bg е част от <span style={{background:'#FFD60A',color:'black',padding:'2px 8px',borderRadius:'10px',fontWeight:'bold'}}>dropoffpay.co.uk</span></div>
      ...
      <div style={contentStyle}>
        ...
        {tab==='my' && <div style={{padding:'12px',display:'flex',flexDirection:'column',gap:'12px'}}>{myRides.map(r=>(
          <div key={r.id} style={{border:'2px solid #0F4C75',borderRadius:'16px',padding:'14px',background:'#f8f9fa'}}>
            ...
            <div style={{fontSize:'12px',color:'#555',marginTop:'4px'}}>🕒 Тръгване: {r.time} • Очакв. пристигане: {r.returnTime} • {r.date} • 👥 {r.seats} места</div>
        ...
        {tab==='offer' && (
          <div style={{padding:'16px',display:'flex',flexDirection:'column',gap:'12px'}}>
            ...
            <div style={{fontSize:'10px',color:'#666',display:'flex',gap:'8px',marginTop:'-8px'}}><span style={{flex:1,textAlign:'center'}}>{t.BG.dateLabel}</span><span style={{flex:1,textAlign:'center'}}>{t.BG.departLabel}</span><span style={{flex:1,textAlign:'center'}}>{t.BG.arrivalLabel}</span></div>

            <div style={{display:'flex',gap:'8px'}}>
              <input placeholder={t.BG.brand} value={offerForm.carBrand} onChange={e=>setOfferForm({...offerForm,carBrand:e.target.value})} style={inputStyle(!!offerForm.carBrand.trim())}/>
              <input placeholder={t.BG.color} value={offerForm.carColor} onChange={e=>setOfferForm({...offerForm,carColor:e.target.value})} style={inputStyle(!!offerForm.carColor.trim())}/>
            </div>
            <input placeholder={t.BG.reg} value={offerForm.carReg} onChange={e=>setOfferForm({...offerForm,carReg:e.target.value.toUpperCase()})} style={inputStyle(!!offerForm.carReg.trim())}/>

            <div>
              <input placeholder={t.BG.seatsPlaceholder} value={offerForm.seats} onChange={e=>setOfferForm({...offerForm,seats:e.target.value})} style={{width:'100%',padding:'12px',borderRadius:'12px',fontSize:'16px',border:seatsFilled?'2px solid #2ECC71':'2px solid #FF3B30',background:seatsFilled?'#e6f9ed':'#fff5f5',outline:'none'}}/>
              <div style={{fontSize:'10px',color:'#666',marginTop:'4px',marginLeft:'4px'}}>{t.BG.seatsDesc} - брой свободни места в колата</div>
            </div>

            <textarea placeholder={t.BG.message} value={offerForm.message} onChange={e=>setOfferForm({...offerForm,message:e.target.value})} style={{padding:'12px',borderRadius:'12px',border:'1px solid #ddd',minHeight:'70px',fontSize:'16px'}}/>
            ...
            {offerForm.type==='offer' && (
              <label style={{display:'flex',gap:'10px',background:offerForm.isDriver?'#e6f9ed':'#fff8e1',padding:'12px',borderRadius:'12px',border:`2px solid ${offerForm.isDriver?'#2ECC71':'#FFD60A'}`}}>
                ...
                <span style={{fontSize:'12px'}}>{t.BG.driverConfirm}</span>
              </label>
            )}
        ...
        {tab==='admin' && isAdmin && <div style={{padding:'12px'}}>Админ панел - {reports.length} сигнала</div>}
      </div>
      <Footer/>
    </main>
  );
