...
const STATIC_CONTENT = {
  bg: {
    ...
    terms: `1. Какво е VoziMe? VoziMe.bg е проект на DropOffPay.co.uk. Информационна платформа. Не е превозвач.\n2. Декларация на шофьора\n3. Плащане - Сайтът не обработва плащания.`,
...
export default function Home(){
  ...
  // === ОПРАВЕН loadMyShopProfile - ВИЖДА ВСИЧКИ СТОКИ ДОРИ АКО ТЕЛЕФОНА Е +359 ИЛИ 0888 ===
  const loadMyShopProfile = async ()=>{
    if(!currentUser?.phone) return;
    const myClean10 = clean(currentUser.phone);
    const myCleanFull = cleanFull(currentUser.phone);
    console.log('Търся магазин за:', currentUser.phone, 'clean10:', myClean10, 'cleanFull:', myCleanFull);
    const {data:allProfiles}=await supabase.from('shop_profiles').select('*');
    if(!allProfiles || allProfiles.length===0){ console.log('Няма shop_profiles'); setMyShopProfile(null); return; }
    // Търси всички съвпадения - по-гъвкаво
    const myProfiles = allProfiles.filter((p:any)=>{
      const pClean10 = clean(p.phone);
      const pCleanFull = cleanFull(p.phone);
      return (
        pClean10===myClean10 ||
        pCleanFull===myCleanFull ||
        p.phone===currentUser.phone ||
        pCleanFull.endsWith(myClean10) ||
        myCleanFull.endsWith(pClean10) ||
        pCleanFull.includes(myClean10) ||
        myCleanFull.includes(pClean10)
      );
    });
    console.log('Намерени профили за мен:', myProfiles);
    if(myProfiles.length===0){ setMyShopProfile(null); return; }
    const ownerProfile = myProfiles.find((p:any)=>p.role==='shop_owner') || myProfiles[0];
    setMyShopProfile(ownerProfile);
    // Вземи всички shop_id-та на този потребител (може да има няколко магазина)
    const shopIds = [...new Set(myProfiles.map((p:any)=>p.shop_id))];
    console.log('Shop IDs:', shopIds);
    let allProds:any[] = [];
    let allOrders:any[] = [];
    for(const sid of shopIds){
      const {data:prods, error:prodErr}=await supabase.from('products').select('*').eq('shop_id', sid).order('created_at',{ascending:false});
      console.log(`Продукти за shop ${sid}:`, prods, 'грешка:', prodErr);
    ...
    // FALLBACK: ако все още 0, търси по име HALASTRA2 директно в shops и вземи продуктите му
    if(allProds.length===0){
      console.log('Fallback търсене по име HALASTRA2');
      const {data:shopsByName}=await supabase.from('shops').select('*').ilike('name','%HALASTRA%');
      console.log('Магазини с име HALASTRA:', shopsByName);
      if(shopsByName && shopsByName.length>0){
        for(const s of shopsByName){
          const {data:prods}=await supabase.from('products').select('*').eq('shop_id', s.id).order('created_at',{ascending:false});
          console.log(`Fallback продукти за ${s.name} ${s.id}:`, prods);
          if(prods && prods.length>0){
            allProds = [...allProds, ...prods];
            // Ако намерим, вземи този shop като мой
            if(!myProfiles.find((p:any)=>p.shop_id===s.id)){
              console.log('Намерих продукти в друг shop, добавям профил');
              setMyShopProfile({shop_id: s.id, role: 'shop_owner', phone: currentUser.phone});
            }
          }
        }
      }
    }
    console.log('Финални продукти за собственика:', allProds);
  ...
  const toggleShopActive = async (shop:any)=>{
    ...
    if(error){ alert('Грешка СПРИ бутон: '+error.message+'\nПусни FIX_RLS_shops.sql!'); return; }
    await loadShops();
  };
      ...
      <div style={{flex:1,overflowY:'auto',background:'#f9fafb'}} ref={ridesContainerRef}>
        ...
        {tab==='shops' && isAdmin && (
          <div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'12px'}}>
            <div style={{background:'#22c55e',color:'white',padding:'12px',borderRadius:'12px'}}><b>👑 Админ Магазини</b><div style={{fontSize:'11px'}}>СПРИ бутон, смяна номер, шофьори.</div></div>
            ...
            {shops.map((s:any)=>{
              ...
              const edit = editShopData[s.id] || {name:s.name, fee:(s.delivery_fee||4.99).toString(), ownerPhone:s.phone||'', newDriverPhone:''};
              return (
                <div key={s.id} style={{background:'white',padding:'12px',borderRadius:'12px',border:s.vip_active===false?'3px solid red':'2px solid #22c55e'}}>
                  ...
                  <div style={{marginTop:'8px',background:'#f9fafb',padding:'8px',borderRadius:'8px'}}>
                    <input value={edit.name} onChange={e=>setEditShopData({...editShopData, [s.id]: {...edit, name:e.target.value}})} placeholder="Име" style={{width:'100%',padding:'6px',borderRadius:'6px',border:'1px solid #ddd',fontSize:'11px'}}/>
                    <div style={{display:'flex',gap:'6px',marginTop:'6px'}}><input value={edit.fee} onChange={e=>setEditShopData({...editShopData, [s.id]: {...edit, fee:e.target.value}})} placeholder="Такса €" style={{flex:1,padding:'6px',borderRadius:'6px',border:'1px solid #ddd',fontSize:'11px'}}/><input value={edit.ownerPhone} onChange={e=>setEditShopData({...editShopData, [s.id]: {...edit, ownerPhone:e.target.value}})} placeholder="Тел собственик" style={{flex:1,padding:'6px',borderRadius:'6px',border:'1px solid #ddd',fontSize:'11px'}}/></div>
                    <button onClick={()=>updateShopInfo(s)} style={{width:'100%',marginTop:'6px',padding:'6px',background:'#0F4C75',color:'white',border:'none',borderRadius:'6px',fontSize:'10px',fontWeight:'bold'}}>💾 Запази</button>
                  </div>
                  <div style={{marginTop:'8px'}}>{drivers.map((d:any)=><div key={d.id} style={{fontSize:'11px',background:'#e0f2fe',padding:'4px',borderRadius:'4px',marginTop:'4px',display:'flex',justifyContent:'space-between'}}><span>🚚 {d.phone}</span><button onClick={()=>removeProfile(d.id)} style={{background:'#FF3B30',color:'white',border:'none',padding:'2px 6px',borderRadius:'4px',fontSize:'9px'}}>X</button></div>)}<div style={{display:'flex',gap:'6px',marginTop:'6px'}}><input value={edit.newDriverPhone} onChange={e=>setEditShopData({...editShopData, [s.id]: {...edit, newDriverPhone:e.target.value}})} placeholder="Нов шофьор" style={{flex:1,padding:'6px',borderRadius:'6px',border:'1px solid #ddd',fontSize:'10px'}}/><button onClick={()=>addDriverToShop(s)} style={{background:'#0F4C75',color:'white',border:'none',padding:'6px 10px',borderRadius:'6px',fontSize:'10px'}}>➕</button></div></div>
                </div>
              )
            })}
          </div>
        )}
        {tab==='admin' && isAdmin && (<div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'12px'}}><div style={{background:'#0F4C75',color:'white',padding:'12px',borderRadius:'12px'}}><div style={{fontWeight:'bold',fontSize:'14px'}}>👑 ADMIN</div><button onClick={syncCurrentUser} style={{marginTop:'8px',background:'#2ECC71',border:'none',padding:'6px 12px',borderRadius:'8px',fontSize:'11px',fontWeight:'bold'}}>{t.refresh}</button></div></div>)}
          ...
          {myShopProfile && myShopProfile.role==='shop_owner' && (
            <div style={{background:'white',border:'3px solid #22c55e',borderRadius:'14px',padding:'14px'}}>
              <div style={{background:'#22c55e',color:'white',padding:'10px',borderRadius:'10px',margin:'-14px -14px 12px -14px'}}>
                <div style={{fontWeight:'800',fontSize:'15px'}}>🛒 Моят магазин - {marketShops.find(s=>s.id===myShopProfile.shop_id)?.name||shops.find(s=>s.id===myShopProfile.shop_id)?.name||'Магазин'} - ти си собственик</div>
                <div style={{fontSize:'11px'}}>Тук променяш стоките си - добавяш, триеш, сменяш цени € и снимки. ID: {myShopProfile.shop_id?.slice(0,8)}</div>
              </div>
              <div style={{background:editingProduct?'#fef3c7':'#f0fdf4',border:`2px solid ${editingProduct?'#f59e0b':'#22c55e'}`,borderRadius:'12px',padding:'12px'}}>
                <div style={{fontWeight:'bold',fontSize:'13px'}}>{editingProduct?`✏️ Редактираш: ${editingProduct.name} - смени цена`:`➕ Добави нова стока`}</div>
                ...
                <div style={{marginTop:'8px',border:'1px dashed #22c55e',padding:'8px',borderRadius:'8px',background:'white'}}>
                  <div style={{fontSize:'11px',fontWeight:'bold'}}>📸 Снимка:</div>
                  {editingProduct?.image_url && !newProdFile && <img src={editingProduct.image_url} style={{width:'50px',height:'50px',objectFit:'cover',borderRadius:'6px',marginTop:'4px'}}/>}
                  <input type="file" accept="image/*" onChange={e=>setNewProdFile(e.target.files?.[0]||null)} style={{width:'100%',marginTop:'4px'}}/>
                </div>
                <div style={{display:'flex',gap:'8px',marginTop:'10px'}}>
                  <button onClick={addProductWithImage} style={{flex:1,padding:'14px',background:editingProduct?'#f59e0b':'#22c55e',color:'white',border:'none',borderRadius:'10px',fontWeight:'bold',fontSize:'14px'}}>{editingProduct?'💾 Запази промените':'➕ Добави продукт с цена €'}</button>
              ...
              <div style={{marginTop:'16px'}}>
                <div style={{fontWeight:'800',fontSize:'13px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                  <span>📋 Моите стоки - {myShopProducts.length}</span>
                  <button onClick={()=>loadMyShopProfile()} style={{background:'#0F4C75',color:'white',border:'none',padding:'4px 10px',borderRadius:'6px',fontSize:'10px'}}>🔄 Опресни</button>
                </div>
                {myShopProducts.length===0 && <div style={{textAlign:'center',padding:'20px',background:'#ffeaea',borderRadius:'8px',marginTop:'8px',fontSize:'12px',border:'1px solid red'}}>⚠️ Нямаш стоки - но в Пазар има 2! Цъкни 🔄 Опресни горе. Ако пак 0 - пусни FIX_RLS. Твоя shop_id: {myShopProfile.shop_id}<br/>Тел: {currentUser.phone}</div>}
                {myShopProducts.map((p:any)=><div key={p.id} style={{display:'flex',gap:'10px',alignItems:'center',border:'1px solid #e5e7eb',padding:'10px',borderRadius:'10px',marginTop:'8px',background:p.id===editingProduct?.id?'#fef3c7':'white'}}>{p.image_url && <img src={p.image_url} style={{width:'55px',height:'55px',objectFit:'cover',borderRadius:'8px',border:'1px solid #eee'}}/>}<div style={{flex:1}}><b style={{fontSize:'13px'}}>{p.name}</b><br/><span style={{fontSize:'11px',color:'#666'}}>{p.description||''}</span><br/><b style={{color:'#22c55e',fontSize:'14px'}}>{p.price}€</b><div style={{fontSize:'9px',color:'#999'}}>ID: {p.id.slice(0,8)} - Shop: {p.shop_id.slice(0,8)}</div></div><div style={{display:'flex',flexDirection:'column',gap:'4px'}}><button onClick={()=>startEditProduct(p)} style={{background:'#0F4C75',color:'white',border:'none',padding:'8px 12px',borderRadius:'8px',fontSize:'11px',fontWeight:'bold'}}>✏️ Промени цена</button><button onClick={()=>deleteProduct(p.id)} style={{background:'#FF3B30',color:'white',border:'none',padding:'6px 12px',borderRadius:'8px',fontSize:'10px'}}>🗑️ Трий</button></div></div>)}
              </div>
            </div>
          )}
          {!myShopProfile && <div style={{background:'white',border:'1px dashed #22c55e',padding:'12px',borderRadius:'12px',fontSize:'12px',color:'#666',textAlign:'center'}}>Нямаш магазин. Твоят телефон: {currentUser.phone}<br/>Админът трябва да те добави в 👑 Магазини по този телефон.</div>}
        </div>}
      </div>
    </main>
  );
