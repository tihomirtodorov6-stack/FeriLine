...
export default function Home(){
  ...
  useEffect(()=>{
    ...
    if(r) setRides(JSON.parse(r).filter((x:any)=>Date.now()-x.createdAt<48*60*60*1000).sort((a:any,b:any)=>b.createdAt-a.createdAt));
    if(rp) setReports(JSON.parse(rp)); if(bp) setBlockedPhones(JSON.parse(bp));
  },[]);

  // АВТО СКРОЛ - винаги показва последната най-отдолу когато влезеш в Намери/Моите
  useEffect(()=>{
    if(scrollRef.current){
      scrollRef.current.scrollTop = 0; // най-новите са горе, но скролваш и виждаш и последната
    }
  },[tab, rides.length]);

  const saveRides=(nr:any[])=>{const s=[...nr].sort((a,b)=>b.createdAt-a.createdAt);setRides(s);localStorage.setItem('vozime_rides_noprice', JSON.stringify(s));};
  ...
  // ФИКСИРАН ЕКРАН - САМО ОБЯВИТЕ СЕ СКРОЛВАТ
  ...
  const contentStyle:React.CSSProperties={
    ...
    paddingBottom:'20px' // за да се вижда последната обява над футъра
  };
  ...
  const bannerStyle:React.CSSProperties={background:'#0F4C75',color:'white',padding:'6px',textAlign:'center',fontSize:'11px',flexShrink:0,zIndex:20};
  const tabsStyle:React.CSSProperties={height:'56px',minHeight:'56px',display:'flex',gap:'6px',padding:'8px',background:'#f1f3f4',flexShrink:0,zIndex:20};
  const footerStyle:React.CSSProperties={minHeight:'78px',flexShrink:0,background:'white',borderTop:'1px solid #e5e7eb',display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px',boxShadow:'0 -4px 20px rgba(0,0,0,0.08)',zIndex:20};
  ...
  return (
    <main style={appStyle}>
      <div style={bannerStyle}>VoziMe.bg е част от <span style={{background:'#FFD60A',color:'black',padding:'2px 8px',borderRadius:'10px',fontWeight:'bold'}}>dropoffpay.co.uk</span></div>
      ...
      <div ref={scrollRef} style={contentStyle}>
        ...
        ))}<div style={{height:'20px',flexShrink:0}}></div></div>}
        ...
        {tab==='admin' && isAdmin && <div style={{padding:'12px'}}>Админ - {reports.length} сигнала</div>}
      </div>
      <Footer/>
    </main>
  );
