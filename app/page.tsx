...
type Lang = 'BG' | 'EN';
const t = {
  BG: { appName:'VoziMe.bg', route:'Полско Косово ↔ Бяла', login:'Вход', register:'Регистрация', logout:'Изход', find:'Намери', my:'Моите', offer:'Предложи', edit:'Редактирай', from:'От', to:'До', departure:'Тръгване', return:'Връщане', seats:'Места', message:'Къде точно минаваш?', date:'Кога', today:'Днес', tomorrow:'Утре', publish:'Публикувай', save:'Запази', cancel:'Откажи', full:'ПЪЛНА', free:'Свободна', markFull:'Пълна', requests:'Заявки', approve:'Одобри', call:'Обади се', viber:'Viber', noRides:'Няма пътувания', noMyRides:'Нямаш обяви', limit:'Лимит 2 обяви за 24ч. Премиум 2.50€/месец' },
  EN: { appName:'VoziMe.bg', route:'Polsko Kosovo ↔ Byala', login:'Login', register:'Register', logout:'Logout', find:'Find', my:'My Rides', offer:'Offer', edit:'Edit', from:'From', to:'To', departure:'Depart', return:'Return', seats:'Seats', message:'Route details?', date:'When', today:'Today', tomorrow:'Tomorrow', publish:'Publish', save:'Save', cancel:'Cancel', full:'FULL', free:'Free', markFull:'Full', requests:'Requests', approve:'Approve', call:'Call', viber:'Viber', noRides:'No rides', noMyRides:'No rides', limit:'Limit 2 rides / 24h. Premium 2.50€/month' }
};
...
export default function Home() {
  ...
  const [offerForm, setOfferForm] = useState({from:'Полско Косово', to:'Бяла', time:'07:30', returnTime:'17:00', date:'Днес', seats:'3', message:'Минавам през паметника в 8:00, жп спирка 8:05'});

  useEffect(()=>{
    const savedLang = localStorage.getItem('vozime_lang') as Lang; if(savedLang) setLang(savedLang);
    const u = localStorage.getItem('vozime_users'); const cu = localStorage.getItem('vozime_current'); let r = localStorage.getItem('vozime_rides_noprice');
    if(u) setUsers(JSON.parse(u)); if(cu) { setCurrentUser(JSON.parse(cu)); setMode('app'); }
    if(r){ const filtered = JSON.parse(r).filter((x:Ride)=>Date.now()-x.createdAt < 48*60*60*1000); localStorage.setItem('vozime_rides_noprice', JSON.stringify(filtered)); setRides(filtered); }
  },[]);
  ...
  const publishRide = ()=>{
    ...
    if(editingRide){ saveRides(rides.map(r=>r.id===editingRide?nr:r)); setEditingRide(null); } else saveRides([nr,...rides]); setOfferForm({from:'Полско Косово', to:'Бяла', time:'07:30', returnTime:'17:00', date:'Днес', seats:'3', message:''}); setTab('my');
  };
  ...
  const requestRide = (id:string)=>{
    ...
    const nrides = rides.map(r=>{
      ...
      if(r.requests.find(q=>q.passengerPhone===currentUser.phone)) return r;
      ...
      <div style={contentStyle}>
          ...
          <div style={{padding:'12px',display:'flex',flexDirection:'column',gap:'12px'}}>
            {rides.filter(r=>!r.isFull).map(ride=>(
              <div key={ride.id} style={{border:'1px solid #eee',borderRadius:'16px',padding:'14px'}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><div style={{fontWeight:'bold',fontSize:'16px'}}>{ride.from} → {ride.to} <span style={{fontSize:'11px',background:'#f1f3f4',padding:'3px 8px',borderRadius:'10px',marginLeft:'6px'}}>{ride.date}</span></div><div style={{fontSize:'12px',color:'#888'}}>{ride.seats} {tr.seats}</div></div>
                ...
                <div style={{fontSize:'10px',color:'#888',textAlign:'center',marginTop:'8px'}}>Обади се за да се разберете</div>
              </div>
            ))}
            {rides.filter(r=>!r.isFull).length===0 && <div style={{textAlign:'center',color:'#888',marginTop:'30px'}}>{tr.noRides}<br/><span style={{fontSize:'11px'}}>Пусни първата обява</span></div>}
          </div>
        )}
          ...
          <div style={{padding:'12px',display:'flex',flexDirection:'column',gap:'12px'}}>
            {myRides.map(ride=>(
              <div key={ride.id} style={{border: ride.isFull? '2px solid red' : '2px solid #0F4C75',borderRadius:'16px',padding:'14px', background: ride.isFull? '#fff5f5' : 'white'}}>
                <div style={{fontWeight:'bold',fontSize:'14px'}}>{ride.date}: {ride.from}→{ride.to} • {ride.time}/{ride.returnTime} • {ride.seats} места {ride.isFull && `🔴 ${tr.full}`}</div>
        ...
        {tab==='offer' && (
          <div style={{padding:'16px',display:'flex',flexDirection:'column',gap:'10px'}}>
            ...
            <div style={{background:'#f1f3f4',padding:'10px',borderRadius:'10px',fontSize:'11px',color:'#666',textAlign:'center'}}>Без търговия • Споделено пътуване • Обади се и се разберете</div>
          </div>
        )}
      </div>
    </main>
