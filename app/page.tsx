...
export default function Home(){
  ...
  const loadRides = async ()=>{
    ...
    const {data, error} = await supabase.from('rides').select('*').gte('created_at', cutoffIso).order('created_at',{ascending:true});
  ...
  const handleAuth = async ()=>{
    ...
    if(loginTab==='register'){
      ...
      const newUser = {id: Date.now().toString(), first_name: loginForm.firstName.trim(), last_name: loginForm.lastName.trim(), phone: loginForm.phone.trim(), clean_phone: cleanPhoneVal};
  ...
  const publishRide= async ()=>{
    ...
    const row:any = {id, driver_name:`${currentUser.firstName} ${currentUser.lastName}`, driver_phone:currentUser.phone, driver_id:currentUser.id, from_city:offerForm.from, to_city:offerForm.to, time:offerForm.time, return_time:offerForm.returnTime, date:lang==='bg'?'Днес':'Today', seats:parseInt(offerForm.seats)||1, message:offerForm.message, created_at:nowIso, type:offerForm.type, is_driver:offerForm.isDriver, car_brand:offerForm.carBrand, car_color:offerForm.carColor, car_reg:offerForm.carReg.toUpperCase(), car_info:`${offerForm.carBrand} ${offerForm.carColor} ${offerForm.carReg.toUpperCase()}`, from_country:offerForm.fromCountry, to_country:offerForm.toCountry};
    try{
      if(editingRide){ await callAdmin('update_ride',{ride_id:editingRide, newData:row}); }
      else { const {error} = await supabase.from('rides').insert(row); if(error) throw error; }
