      ...
      <div style={{flex:1,overflowY:'auto',background:'#f9fafb'}} ref={ridesContainerRef}>
        ...
        {tab==='shops' && isAdmin && (
          <div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'12px'}}>
            ...
            {shops.map((s:any)=><div key={s.id} style={{background:'white',padding:'12px',borderRadius:'8px',border:s.vip_active===false?'2px solid red':'1px solid #eee',display:'flex',justifyContent:'space-between',alignItems:'center'}}><div><b>{s.name}</b> - {s.phone}<br/><span style={{fontSize:'11px'}}>{s.delivery_fee}лв при магазина - {s.vip_active===false?'🔴 СПРЯН':'🟢 Активен'}</span></div><div style={{display:'flex',flexDirection:'column',gap:'4px'}}><button onClick={()=>toggleShopActive(s)} style={{background:s.vip_active===false?'#22c55e':'#FF3B30',color:'white',border:'none',padding:'6px 12px',borderRadius:'6px',fontSize:'10px',fontWeight:'bold'}}>{s.vip_active===false?'✅ ПУСНИ':'⛔ СПРИ'}</button></div></div>)}
          </div>
        )}
        ...
        {tab==='my' && <div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'10px'}}>
          ...
          {myShopProfile && myShopProfile.role==='driver' && (
            <div style={{background:'white',border:'2px solid #0F4C75',borderRadius:'12px',padding:'12px'}}>
              ...
              <div style={{marginTop:'10px'}}>{myShopOrders.map((o:any)=><div key={o.id} style={{background:'#f9fafb',border:'1px solid #0F4C75',padding:'10px',borderRadius:'8px',marginBottom:'8px',fontSize:'12px'}}><b>Поръчка {o.id.slice(0,8)}</b> - {o.status}<br/>{o.customer_name} - {o.customer_phone}<br/>📍 {o.address}<br/>🛒 {o.items?.map((it:any)=>it.name).join(', ')}<br/><b>Тотал {o.total}лв</b><div style={{display:'flex',gap:'6px',marginTop:'8px'}}><a href={`tel:${o.customer_phone}`} style={{flex:1,background:'#0F4C75',color:'white',padding:'8px',borderRadius:'6px',textAlign:'center',textDecoration:'none'}}>Обади се</a><button onClick={()=>updateOrderStatus(o.id,'on_the_way')} style={{flex:1,background:'#FFD60A',border:'none',padding:'8px',borderRadius:'6px',fontWeight:'bold'}}>Пътувам</button><button onClick={()=>updateOrderStatus(o.id,'delivered')} style={{flex:1,background:'#22c55e',color:'white',border:'none',padding:'8px',borderRadius:'6px',fontWeight:'bold'}}>Доставено</button></div></div>)}</div>
            </div>
          )}
          {!myShopProfile && <div style={{background:'white',border:'1px dashed #22c55e',padding:'12px',borderRadius:'12px',fontSize:'12px',color:'#666',textAlign:'center'}}>Нямаш магазин. Админът трябва да те добави по телефон в 👑 Магазини. Телефона трябва да съвпада - 0888123456 или +447...<br/>Твоят телефон сега: {currentUser.phone}<br/>Ако си магазинер и не виждаш панела - пиши на админа да те добави пак с точния ти телефон.</div>}
