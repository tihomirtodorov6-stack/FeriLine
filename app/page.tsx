...
const EDGE_URL = "https://ypfbljjrpppkdxdftjcv.supabase.co/functions/v1";
...
export default function Home(){
  ...
  const callEdge = async (func:string, body:any)=>{
    const res = await fetch(`${EDGE_URL}/${func}`, {
      method: 'POST',
      headers: { 'Content-Type':'application/json', 'Authorization': `Bearer ${SUPABASE_ANON_KEY}`, 'apikey': SUPABASE_ANON_KEY },
      body: JSON.stringify({...body, adminPhone: currentUser?.phone, phone: currentUser?.phone})
    });
  ...
  useEffect(()=>{
    ...
    const interval = setInterval(()=>{ loadMaintenance(); loadBans(); loadReports(); loadRides(); }, 2000);
  ...
  const toggleMaintenance = async ()=>{
    ...
    try{
      await callEdge('toggle-maintenance', { enabled: newEnabled, msg: maintenance.msg });
      setMaintenance({...maintenance,enabled:newEnabled});
    }catch(e:any){ alert('Грешка: '+e.message); }
  };
  const updateMaintenanceMsg = async (msg:string)=>{
    setMaintenance({...maintenance,msg});
    try{ await callEdge('toggle-maintenance', { enabled: maintenance.enabled, msg }); }catch{}
  };
  ...
  const publishRide= async ()=>{
    ...
    try{
      if(editingRide){
        await callEdge('delete-ride', { ride_id: editingRide, driver_id: currentUser.id, action: 'update', newData: row });
      } else {
  ...
  const deleteRide = async (id:string)=>{
    ...
    try{
      await callEdge('delete-ride', { ride_id: id, driver_id: ride?.driverId || currentUser?.id });
      await loadRides();
    }catch(e:any){ alert('Грешка триене: '+e.message); }
  }
  ...
  const handleBan = async (phone:string, reason:string, duration:string)=>{
    try{
      await callEdge('ban-user', { driver_id: clean(phone), original_phone: phone, reason, duration });
      await loadBans(); setBanPhoneInput(''); setBanReasonInput('');
    }catch(e:any){ alert('Грешка бан: '+e.message); }
  };
  const unban = async (id:string)=>{
    try{ await callEdge('unban-user', { ban_id: id }); setBans(bans.filter(b=>b.id!==id)); }catch(e:any){ alert(e.message); }
  };
  const dismissReport = async (id:string)=>{
    try{ await callEdge('dismiss-report', { report_id: id, status:'dismissed' }); setReports(reports.filter(r=>r.id!==id)); }catch(e:any){ alert(e.message); }
  };
  const banFromReport = async (rep:any, dur:string)=>{
    await handleBan(rep.reportedPhone||rep.reported_phone, `Репорт: ${rep.reason}`, dur);
    try{ await callEdge('dismiss-report', { report_id: rep.id, status:'banned' }); setReports(reports.filter(r=>r.id!==rep.id)); }catch{}
  };
      ...
      <div style={{flex:1,overflowY:'auto',background:'#f9fafb'}} ref={ridesContainerRef}>
        ...
        {tab==='admin' && isAdmin && (
          <div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'12px'}}>
            <div style={{background:'#0F4C75',color:'white',padding:'12px',borderRadius:'12px'}}><div style={{fontWeight:'bold',fontSize:'14px'}}>👑 АДМИН - {currentUser.phone} • Защитен с Edge Functions ✅</div><div style={{fontSize:'11px',opacity:0.8}}>RLS + Service Role - хакери не могат да трият</div></div>
