...
export default function Home(){
  ...
  const loadShops = async ()=>{ const {data}=await supabase.from('shops').select('*').order('created_at',{ascending:false}); if(data){ setShops(data); setMarketShops(data.filter((s:any)=>s.vip_active!==false)); } };
  const loadMyShopProfile = async ()=>{
    ...
    const {data:prods}=await supabase.from('products').select('*').eq('shop_id', match.shop_id); if(prods) setMyShopProducts(prods);
  ...
  const toggleShopActive = async (shop:any)=>{
    ...
    await supabase.from('shops').update({vip_active:newVal}).eq('id', shop.id);
    await loadShops();
  };
  // ===== ОПРАВЕНА ФУНКЦИЯ ЗА ЦЕНА - приема 6,25 и 6.25лв =====
  const addProductWithImage = async ()=>{
    if(!myShopProfile?.shop_id){ alert('Нямаш магазин!'); return; }
    if(!newProdName.trim()){ alert('Напиши име на продукта! Пример: Кайма Варна'); return; }
    if(!newProdPrice.trim()){ alert('Напиши цена! Пример: 6.25'); return; }
    // Поправка: приема запетая, точки, лв, интервали
    const cleanedPrice = newProdPrice.replace(',', '.').replace(/[^\d.]/g, '');
    const priceNum = parseFloat(cleanedPrice);
    if(isNaN(priceNum) || priceNum<=0){ alert('Цената е грешна! Напиши примерно: 6.25 или 6,25'); return; }
    let imageUrl = null;
    if(newProdFile){
      ...
      if(upErr){ alert('Грешка качване снимка: '+upErr.message); return; }
    ...
    if(error){ alert('Грешка база: '+error.message); return; }
    ...
    alert('Продукт добавен! Вижда се в Пазар веднага.');
  };
      ...
      <div style={{flex:1,overflowY:'auto',background:'#f9fafb'}} ref={ridesContainerRef}>
        ...
        {tab==='shops' && isAdmin && (
          <div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'12px'}}>
            <div style={{background:'#22c55e',color:'white',padding:'12px',borderRadius:'12px'}}><b>👑 Админ Магазини - само +447935463970</b><div style={{fontSize:'11px'}}>Бутон СПРИ/ПУСНИ - спира достъпа ако не плати.</div></div>
        ...
        {tab==='my' && <div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'10px'}}>
          {myShopProfile && myShopProfile.role==='shop_owner' && (
            <div style={{background:'white',border:'2px solid #22c55e',borderRadius:'12px',padding:'12px'}}>
              <div style={{fontWeight:'bold'}}>🛒 Моят магазин - {marketShops.find(s=>s.id===myShopProfile.shop_id)?.name||'Магазин'} - ти си собственик</div>
              <div style={{fontSize:'11px',marginTop:'4px'}}>Добави лист със стоки + снимки + цени. Пример: Име: Кайма Варна | Цена: 6.25 | Описание: прясно</div>
              <input value={newProdName} onChange={e=>setNewProdName(e.target.value)} placeholder="Име *: Кайма Варна" style={{width:'100%',padding:'12px',borderRadius:'8px',border:'1px solid #22c55e',marginTop:'8px',background:'#f0fdf4'}}/>
              <input value={newProdPrice} onChange={e=>setNewProdPrice(e.target.value)} placeholder="Цена *: 6.25€ или 6,25€" style={{width:'100%',padding:'12px',borderRadius:'8px',border:'1px solid #22c55e',marginTop:'8px',background:'#f0fdf4'}}/>
              ...
              <div style={{marginTop:'8px',border:'1px dashed #22c55e',padding:'8px',borderRadius:'8px'}}><div style={{fontSize:'11px',fontWeight:'bold'}}>📸 Снимка към стоката (по желание):</div><input type="file" accept="image/*" onChange={e=>setNewProdFile(e.target.files?.[0]||null)} style={{width:'100%',marginTop:'4px'}}/></div>
              <button onClick={addProductWithImage} style={{width:'100%',marginTop:'10px',padding:'14px',background:'#22c55e',color:'white',border:'none',borderRadius:'10px',fontWeight:'bold',fontSize:'14px'}}>➕ Добави продукт със снимка и цена</button>
              <div style={{marginTop:'12px'}}>{myShopProducts.map((p:any)=><div key={p.id} style={{display:'flex',gap:'8px',alignItems:'center',borderBottom:'1px solid #eee',padding:'8px 0'}}>{p.image_url && <img src={p.image_url} style={{width:'40px',height:'40px',objectFit:'cover',borderRadius:'6px'}}/>}<span style={{flex:1, fontSize:'12px'}}><b>{p.name}</b> - {p.price}€<br/><span style={{fontSize:'10px'}}>{p.description}</span></span><button onClick={()=>deleteProduct(p.id)} style={{background:'#FF3B30',color:'white',border:'none',padding:'6px 10px',borderRadius:'6px'}}>🗑️</button></div>)}</div>
          ...
          {myShopProfile && myShopProfile.role==='driver' && (
            <div style={{background:'white',border:'2px solid #0F4C75',borderRadius:'12px',padding:'12px'}}>
              <div style={{fontWeight:'bold'}}>🚚 Моите доставки - шофьор си</div>
              <div style={{fontSize:'11px',marginTop:'4px'}}>Получаваш поръчки от магазина към който те е добавил админа.</div>
          ...
          {!myShopProfile && <div style={{background:'white',border:'1px dashed #22c55e',padding:'12px',borderRadius:'12px',fontSize:'12px',color:'#666',textAlign:'center'}}>Нямаш магазин. Админът трябва да те добави по телефон в 👑 Магазини. Твоят телефон: {currentUser.phone}</div>}
