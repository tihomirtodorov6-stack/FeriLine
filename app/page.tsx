...
export default function Home(){
  ...
  const loadAllUsers = async ()=>{
    if(!isAdmin) return;
    const {data} = await supabase.from('users').select('*').order('first_name',{ascending:true}).limit(1000);
    if(data) setAllUsers(data);
  }

  useEffect(()=>{ const savedLang = localStorage.getItem('vozime_lang') as 'bg'|'en'; if(savedLang) setLang(savedLang); const cu=localStorage.getItem('vozime_current'); if(cu) setCurrentUser(JSON.parse(cu)); loadRides(); loadMaintenance(); loadBans(); loadReports(); fetchLocation(); const interval = setInterval(()=>{ loadMaintenance(); loadBans(); loadReports(); loadRides(); if(isAdminPhone(JSON.parse(localStorage.getItem('vozime_current')||'{}').phone||'')) loadAllUsers(); }, 3000); return ()=> clearInterval(interval); },[]);
  useEffect(()=>{ if(currentUser && isAdmin) loadAllUsers(); },[currentUser]);
  ...
  const handleAuth = async ()=>{
    ...
    if(loginTab==='register'){
      ...
      if(existing){
        if(existing.deleted_at){ // възстанови изтрит акаунт
          await supabase.from('users').update({deleted_at:null, first_name:loginForm.firstName.trim(), last_name:loginForm.lastName.trim(), phone:loginForm.phone.trim()}).eq('id',existing.id);
          const appUser = {id:existing.id, firstName:loginForm.firstName.trim(), lastName:loginForm.lastName.trim(), phone:loginForm.phone.trim(), is_verified:existing.is_verified, verification_photo_url:existing.verification_photo_url};
          localStorage.setItem('vozime_current', JSON.stringify(appUser)); setCurrentUser(appUser); return;
        }
        alert('Вече съществува! Влез.'); setLoginTab('login'); return;
      }
      const {data:inserted, error} = await supabase.from('users').insert({
        first_name: loginForm.firstName.trim(),
        last_name: loginForm.lastName.trim(),
        phone: loginForm.phone.trim(),
        clean_phone: cleanPhoneVal,
      ...
      if(found.deleted_at && (Date.now() - Number(found.deleted_at) > 6*30*24*60*60*1000)){ alert('Акаунтът е изтрит преди повече от 6 месеца. Регистрирай се наново.'); await supabase.from('users').delete().eq('id',found.id); setLoginTab('register'); return; }
      if(found.deleted_at){ // ако е изтрит но е в 6-те месеца, възстанови го
        await supabase.from('users').update({deleted_at:null}).eq('id',found.id);
      }
      const appUser = {id:found.id, firstName:found.first_name, lastName:found.last_name, phone:found.phone, is_verified:found.is_verified, verification_photo_url:found.verification_photo_url, country_code:found.country_code};
      localStorage.setItem('vozime_current', JSON.stringify(appUser)); setCurrentUser(appUser);
    }
  };
  ...
  const handleVerificationUpload = async ()=>{
    ...
    try{
      ...
      alert('Снимката е изпратена! Админ ще я одобри до 24ч.');
      setVerifyFile(null);
      const updatedUser = {...currentUser, verification_photo_url:urlData.publicUrl, is_verified:false};
      localStorage.setItem('vozime_current', JSON.stringify(updatedUser));
      setCurrentUser(updatedUser);
    }catch(e:any){ alert(e.message); } finally{ setUploadingVerify(false); }
  }

  const handleDeleteAccount = async ()=>{
    if(!confirm('Сигурен ли си? Акаунтът ще се пази 6 месеца за сигурност и после ще се изтрие завинаги.')) return;
    try{
      await supabase.from('users').update({deleted_at: Date.now()}).eq('id', currentUser.id);
      // не трием rides веднага, остават за разследване
    }catch{}
  ...
  const deleteRide = async (id:string)=>{
    try{
      ...
      if(isOwner){
        ...
        await callAdmin('delete_ride',{ride_id:id});
      } else {
        alert('Нямаш право!');
        return;
      }
  ...
  const filteredUsers = allUsers.filter(u=>{
    ...
    return (u.first_name?.toLowerCase().includes(s) || u.last_name?.toLowerCase().includes(s) || u.phone?.includes(s) || u.clean_phone?.includes(s));
  }).sort((a,b)=>{
    const ca = a.country_code||getCountryByPhone(a.phone||'');
    const cb = b.country_code||getCountryByPhone(b.phone||'');
    ...
    return (
      <main style={{position:'fixed',inset:0,display:'flex',alignItems:'center',justifyContent:'center',background:'#0F4C75',padding:'16px',boxSizing:'border-box'}}>
        <div style={{background:'white',padding:'20px',borderRadius:'24px',width:'100%',maxWidth:'380px',boxSizing:'border-box'}}>
          ...
          <div style={{fontSize:'10px',color:'#666',marginTop:'10px',textAlign:'center'}}>Регистрирайки се се съгласяваш снимката за верификация да се пази 1г за сигурност.</div>
        </div>
      </main>
    );
  }
  ...
  return (
    <main style={{height:'100dvh',width:'100%',maxWidth:'480px',margin:'0 auto',background:'white',display:'flex',flexDirection:'column',overflow:'hidden',fontFamily:'-apple-system, sans-serif'}}>
      <div style={{flexShrink:0}}>
        <div style={{background:maintenance.enabled?'#FF3B30':'#0F4C75',color:'white',padding:'6px',textAlign:'center',fontSize:'10px'}}>{maintenance.enabled?'🔴 MAINTENANCE - Сайтът е спрян!':'🌍 VoziMe • 100% безплатна платформа • Сигурна ✅'}</div>
      ...
      {!isVerified && tab!=='my' && <div style={{background:'#FFD60A',color:'black',padding:'8px',textAlign:'center',fontSize:'11px',fontWeight:'bold',flexShrink:0}}>{t.needVerify} - Влез в Моите -> Верифицирай се!</div>}
