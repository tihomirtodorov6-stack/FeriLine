  ...
  return (
    <main style={appStyle}>
      ...
      <div style={contentStyle}>
              ...
              <div key={ride.id} style={{border: isOffer? '1px solid #eee' : '2px dashed #0F4C75',borderRadius:'16px',padding:'14px',background: isOffer? 'white' : '#f0f7ff'}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:'8px'}}>
                  ...
                  <div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:'4px'}}>
                    <span style={{fontSize:'10px',padding:'4px 8px',borderRadius:'10px',fontWeight:'bold',whiteSpace:'nowrap',background: isOffer? '#e6f9ed' : '#dbeafe', color: isOffer? '#0F4C75' : '#0F4C75'}}>{isOffer? tr.driverBadge : tr.passengerBadge}</span>
        ...
        {tab==='my' && (<div style={{padding:'12px',display:'flex',flexDirection:'column',gap:'12px'}}>{myRides.map(ride=><div key={ride.id} style={{border:'2px solid #0F4C75',borderRadius:'16px',padding:'14px'}}><div style={{fontWeight:'bold'}}>{ride.type==='request'?'🙋':'🚗'} {ride.from}→{ride.to} • {ride.time}</div><div style={{display:'flex',gap:'6px',marginTop:'10px'}}><button onClick={()=>startEdit(ride)} style={{flex:1,background:'#0F4C75',color:'white',border:'none',padding:'8px',borderRadius:'10px'}}>✏️ {tr.edit}</button><button onClick={()=>deleteRide(ride.id)} style={{background:'#eee',border:'none',padding:'8px 12px',borderRadius:'10px'}}>🗑️</button></div></div>)}</div>)}
        {tab==='offer' && (
