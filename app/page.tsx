...
export default function Home(){
  ...
  const handleBan = async (phone:string, reason:string, duration:string)=>{
    try{
      await callEdge('ban-user', { original_phone: phone, reason, duration });
      ...
      <div style={{flex:1,overflowY:'auto',background:'#f9fafb'}} ref={ridesContainerRef}>
        ...
        {tab==='admin' && isAdmin && (
          <div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'12px'}}>
            <div style={{background:'#0F4C75',color:'white',padding:'12px',borderRadius:'12px'}}><div style={{fontWeight:'bold',fontSize:'14px'}}>👑 АДМИН - {currentUser.phone} • Защитен ✅</div><div style={{fontSize:'11px',opacity:0.8}}>RLS + Service Role</div></div>
