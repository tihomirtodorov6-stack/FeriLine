...
export default function Home(){
  ...
  useEffect(()=>{
    ...
    const channel = supabase.channel('site_settings_changes').on('postgres_changes',{event:'*',schema:'public',table:'site_settings'}, (payload:any)=>{
      if(payload.new) setMaintenance({enabled:payload.new.maintenance_enabled, msg:payload.new.maintenance_msg||''});
    }).subscribe();
    const channelBans = supabase.channel('bans_changes').on('postgres_changes',{event:'*',schema:'public',table:'bans'}, ()=>{ loadBans(); }).subscribe();
    const channelReports = supabase.channel('reports_changes').on('postgres_changes',{event:'*',schema:'public',table:'reports'}, ()=>{ loadReports(); }).subscribe();
    return ()=>{ supabase.removeChannel(channel); supabase.removeChannel(channelBans); supabase.removeChannel(channelReports); };
  },[]);
      ...
      <div style={{flex:1,overflowY:'auto',background:'#f9fafb'}} ref={ridesContainerRef}>
        ...
        {tab==='admin' && isAdmin && (
          <div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'12px'}}>
            <div style={{background:'#0F4C75',color:'white',padding:'12px',borderRadius:'12px'}}><div style={{fontWeight:'bold',fontSize:'14px'}}>👑 АДМИН - {currentUser.phone}</div><div style={{fontSize:'11px',opacity:0.8}}>Supabase - глобално!</div></div>
