...
const t={
  BG:{
    ...
    shared:'Част от мрежата на dropoffpay.co.uk • Споделен разход',
    limit:'Лимит 2 обяви за 24ч',
    sharedCost:'Споделен разход',
    donateTitle:'VoziMe.bg е безплатен',
    donateSub:'Част от DropOffPay Network • Свободно за всички',
  ...
  EN:{
    ...
    donateSub:'Part of DropOffPay Network • Free for all',
...
export default function Home(){
  ...
  const [offerForm,setOfferForm]=useState({from:'',to:'',time:'09:30',returnTime:'12:30',date:'Днес',seats:'4',message:''});
  ...
  const Footer=()=>(
    <div style={footerStyle}>
      <div style={{flex:1,minWidth:0}}>
        ...
        <div style={{fontSize:'11px',color:'#666',marginTop:'3px',lineHeight:'1.2'}}>{tr.donateSub}</div>
      </div>
  ...
  if(mode!=='app'){
    return (<main style={appStyle}>
      <div style={networkBannerStyle}>
        <span>{tr.networkBanner}</span>
        <span style={{background:'#FFD60A',color:'black',padding:'2px 8px',borderRadius:'10px',fontWeight:'bold',fontSize:'11px'}}>{tr.networkName}</span>
      </div>
      <div style={{...contentStyle,padding:'24px'}}>
        ...
        <div style={{textAlign:'center',marginTop:'16px'}}>
          ...
          <h1 style={{fontSize:'28px',fontWeight:'bold',margin:'12px 0 4px'}}>{tr.appName}</h1>
          <p style={{fontSize:'13px',color:'#888'}}>{tr.route}</p>
          <div style={{background:'black',color:'#FFD60A',fontSize:'10px',fontWeight:'bold',padding:'6px 12px',borderRadius:'20px',display:'inline-block',marginTop:'10px',letterSpacing:'0.5px'}}>{tr.verified}</div>
          <p style={{fontSize:'11px',color:'#0F4C75',background:'#e6f9ed',display:'inline-block',padding:'4px 10px',borderRadius:'20px',marginTop:'8px'}}>{tr.shared}</p>
        </div>
  ...
  return (
    <main style={appStyle}>
      <div style={networkBannerStyle}>
        <span>{tr.networkBanner}</span>
        <a href="https://dropoffpay.co.uk" target="_blank" style={{background:'#FFD60A',color:'black',padding:'2px 8px',borderRadius:'10px',fontWeight:'bold',fontSize:'11px',textDecoration:'none'}}>{tr.networkName}</a>
      </div>
      ...
      <div style={contentStyle}>
            ...
            {visibleRides.map(ride=>(
              <div key={ride.id} style={{border:'1px solid #eee',borderRadius:'16px',padding:'14px'}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:'8px'}}>
                  <div style={{fontWeight:'bold',fontSize:'16px'}}>{ride.from} → {ride.to} <span style={{fontSize:'11px',background:'#f1f3f4',padding:'3px 8px',borderRadius:'10px',marginLeft:'6px'}}>{translateDate(ride.date,lang)}</span></div>
        ...
        {tab==='my' && (<div style={{padding:'12px',display:'flex',flexDirection:'column',gap:'12px'}}>{myRides.map(ride=><div key={ride.id} style={{border:'2px solid #0F4C75',borderRadius:'16px',padding:'14px'}}><div style={{fontWeight:'bold'}}>{ride.from}→{ride.to} • {ride.time}</div><div style={{display:'flex',gap:'6px',marginTop:'10px'}}><button onClick={()=>startEdit(ride)} style={{flex:1,background:'#0F4C75',color:'white',border:'none',padding:'8px',borderRadius:'10px'}}>✏️ {tr.edit}</button><button onClick={()=>deleteRide(ride.id)} style={{background:'#eee',border:'none',padding:'8px 12px',borderRadius:'10px'}}>🗑️</button></div></div>)}</div>)}
        {tab==='offer' && (
