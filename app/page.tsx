...
export default function Home(){
  ...
  const headerStyle:React.CSSProperties={height:'60px',minHeight:'60px',background:'#0F4C75',color:'white',display:'flex',alignItems:'center',padding:'0 12px',gap:'8px',flexShrink:0,zIndex:10};
  ...
  return (
    <main style={appStyle}>
      ...
      <div style={contentStyle}>
        {tab==='find' && <div style={{padding:'12px',display:'flex',flexDirection:'column',gap:'12px'}}>{visibleRides.map((r:any)=><div key={r.id} style={{border:'1px solid #eee',borderRadius:'16px',padding:'14px'}}><div style={{display:'flex',justifyContent:'space-between'}}><b>{r.from} → {r.to}</b><span style={{background:'#FF3B30',color:'white',fontSize:'10px',padding:'3px 8px',borderRadius:'8px'}}>НЕ Е БЕЗПЛАТНО</span></div><div style={{fontSize:'12px',color:'#666',marginTop:'4px'}}>🕒 Тръгване: {r.time} • Очакв.: {r.returnTime} • {r.date}</div><div style={{fontSize:'12px',marginTop:'4px'}}>🚗 {r.carInfo} • 👥 {r.seats} места</div><div style={{fontSize:'13px',marginTop:'8px'}}>👤 {r.driverName} ✓ ШОФЬОР</div></div>)}</div>}

        {tab==='my' && <div style={{padding:'12px',display:'flex',flexDirection:'column',gap:'12px'}}>{myRides.map((r:any)=>(
