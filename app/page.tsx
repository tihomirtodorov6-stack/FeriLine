...
export default function Home(){
  ...
  // === ФИКС V2 - ГАРАНТИРАНО ЗАРЕЖДА СТОКИТЕ ЗА СОБСТВЕНИКА ===
  const loadMyShopProfile = async ()=>{
    if(!currentUser?.phone){ setDebugMsg('Няма currentUser phone'); return; }
    const myClean10 = clean(currentUser.phone);
    const myCleanFull = cleanFull(currentUser.phone);
    setDebugMsg(`Търся за ${currentUser.phone} -> ${myClean10} / ${myCleanFull}`);
    
    // 1. Вземи всички профили
    const {data:allProfiles, error:profErr}=await supabase.from('shop_profiles').select('*');
    if(profErr){ setDebugMsg(`Грешка profiles: ${profErr.message}`); return; }
    if(!allProfiles || allProfiles.length===0){ setDebugMsg('Няма профили в shop_profiles - пусни RLS FIX!'); setMyShopProfile(null); return; }
    
    // 2. Намери всички мои профили - много гъвкаво
    ...
    if(myProfiles.length===0){
      setDebugMsg(`Няма съвпадение за ${currentUser.phone} в ${allProfiles.length} профила. Профили: ${allProfiles.map((p:any)=>p.phone).join(', ')}`);
      // Fallback: опитай да намериш магазин с име HALASTRA2 и да го присвоиш
      const {data:halShops}=await supabase.from('shops').select('*').ilike('name','%HALASTRA2%');
      if(halShops && halShops.length>0){
        const halShop = halShops[0];
        setMyShopProfile({shop_id: halShop.id, role: 'shop_owner', phone: currentUser.phone});
        // Зареди продуктите му директно
        const {data:prods, error:prodErr}=await supabase.from('products').select('*').eq('shop_id', halShop.id);
        if(prodErr){
          setDebugMsg(`Fallback HALASTRA2 продукти грешка: ${prodErr.message} - ПУСНИ RLS FIX SQL!`);
        } else if(prods){
          setMyShopProducts(prods);
          setDebugMsg(`Fallback намери ${prods.length} продукта за HALASTRA2!`);
          // Зареди и поръчки
          const {data:ords}=await supabase.from('orders').select('*').eq('shop_id', halShop.id).order('created_at',{ascending:false});
    ...
    // 3. Вземи всички shop_id-та
    const shopIds = [...new Set(myProfiles.map((p:any)=>p.shop_id))];
    setDebugMsg(`Намерени ${myProfiles.length} профила, shop_ids: ${shopIds.join(', ').slice(0,50)}`);
    
    // 4. Зареди продукти - ОПИТАЙ 3 НАЧИНА
    let allProds:any[] = [];
    let allOrders:any[] = [];
    
    // Начин 1: по shop_id
    for(const sid of shopIds){
      const {data:prods, error}=await supabase.from('products').select('*').eq('shop_id', sid);
      if(error){
        setDebugMsg(`Грешка продукти за ${sid}: ${error.message} - ПУСНИ RLS FIX!`);
      } else if(prods){
    ...
    // Начин 2: ако още 0, зареди ВСИЧКИ продукти и филтрирай по shop_id ръчно (заобикаля RLS по shop_id ако има)
    if(allProds.length===0){
      setDebugMsg(`0 продукта по shop_id, опитвам всички продукти...`);
      const {data:allProducts, error:allErr}=await supabase.from('products').select('*');
      if(allErr){
        setDebugMsg(`Грешка всички продукти: ${allErr.message} - ПУСНИ RLS FIX SQL В SUPABASE!`);
      } else if(allProducts){
        // Филтрирай по shopIds
        const filtered = allProducts.filter((p:any)=> shopIds.includes(p.shop_id));
        if(filtered.length>0){
          allProds = filtered;
          setDebugMsg(`Намерих ${filtered.length} продукта чрез всички продукти!`);
        } else {
          // Fallback: по име на магазин HALASTRA2
          ...
          if(halShops){
            for(const hs of halShops){
              ...
              if(prodsForHal.length>0){
                allProds = [...allProds, ...prodsForHal];
                setDebugMsg(`Fallback по име HALASTRA намери ${prodsForHal.length} продукта!`);
              }
            }
          }
        }
    ...
    if(allProds.length>0){
      setDebugMsg(`✅ Заредени ${allProds.length} продукта за ${currentUser.phone}!`);
    } else {
      setDebugMsg(`❌ 0 продукта за ${currentUser.phone} shop ${shopIds[0]}. В Пазар има! ПУСНИ RLS FIX SQL!`);
    }
  };
      ...
      <div style={{flex:1,overflowY:'auto',background:'#f9fafb'}} ref={ridesContainerRef}>
        {tab==='market' && (
          <div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'10px'}}>
            {!selectedMarketShop? (
              <>
                <div style={{background:'#22c55e',color:'white',padding:'12px',borderRadius:'12px'}}><b>🛒 Пазар</b><div style={{fontSize:'11px'}}>VoziMe взима 0% - доставката е при магазина.</div></div>
        ...
        {tab==='shops' && isAdmin && (
          <div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'12px'}}>
            <div style={{background:'#22c55e',color:'white',padding:'12px',borderRadius:'12px'}}><b>👑 Админ Магазини</b></div>
            <div style={{background:'white',border:'2px solid #22c55e',borderRadius:'12px',padding:'12px'}}>
              ...
              <input value={ownerPhoneInput} onChange={e=>setOwnerPhoneInput(e.target.value)} placeholder="Собственик" style={{width:'100%',padding:'10px',borderRadius:'8px',border:'1px solid #ddd',marginTop:'8px'}}/>
              <input value={driverPhoneInput} onChange={e=>setDriverPhoneInput(e.target.value)} placeholder="Шофьор" style={{width:'100%',padding:'10px',borderRadius:'8px',border:'1px solid #ddd',marginTop:'8px'}}/>
            ...
            {shops.map((s:any)=><div key={s.id} style={{background:'white',padding:'12px',borderRadius:'12px',border:s.vip_active===false?'3px solid red':'2px solid #22c55e'}}><div style={{display:'flex',justifyContent:'space-between'}}><div><b>{s.name}</b><div style={{fontSize:'11px'}}>{s.vip_active===false?'🔴 СПРЯН':'🟢 Активен'}</div></div><div style={{display:'flex',gap:'4px'}}><button onClick={()=>toggleShopActive(s)} style={{background:s.vip_active===false?'#22c55e':'#FF3B30',color:'white',border:'none',padding:'8px 12px',borderRadius:'8px',fontSize:'11px',fontWeight:'bold'}}>{s.vip_active===false?'✅ ПУСНИ':'⛔ СПРИ'}</button><button onClick={()=>deleteShop(s.id)} style={{background:'black',color:'white',border:'none',padding:'8px 10px',borderRadius:'8px',fontSize:'10px'}}>🗑️</button></div></div></div>)}
          </div>
        )}
          ...
          {myShopProfile && (
            <div style={{background:'white',border:'3px solid #22c55e',borderRadius:'14px',padding:'14px'}}>
              <div style={{background:'#22c55e',color:'white',padding:'10px',borderRadius:'10px',margin:'-14px -14px 12px -14px'}}>
                <div style={{fontWeight:'800',fontSize:'14px'}}>🛒 Моят магазин - {shops.find(s=>s.id===myShopProfile.shop_id)?.name||marketShops.find(s=>s.id===myShopProfile.shop_id)?.name||'Магазин'} - {myShopProfile.role}</div>
                <div style={{fontSize:'10px'}}>ID: {myShopProfile.shop_id?.slice(0,13)}... Тел: {currentUser.phone}</div>
                {debugMsg && <div style={{fontSize:'10px',background:'black',color:'#0f0',padding:'6px',borderRadius:'6px',marginTop:'6px',whiteSpace:'pre-wrap'}}>{debugMsg}</div>}
              </div>
              ...
              <div style={{background:editingProduct?'#fef3c7':'#f0fdf4',border:`2px solid ${editingProduct?'#f59e0b':'#22c55e'}`,borderRadius:'12px',padding:'12px'}}>
                ...
                <input value={newProdPrice} onChange={e=>setNewProdPrice(e.target.value)} placeholder="Цена *: 6.25€" style={{width:'100%',padding:'10px',borderRadius:'8px',border:'2px solid #22c55e',marginTop:'8px',background:'white'}}/>
              ...
              <div style={{marginTop:'14px'}}>
                <div style={{fontWeight:'800',fontSize:'13px',display:'flex',justifyContent:'space-between'}}>
                  ...
                  <button onClick={()=>loadMyShopProfile()} style={{background:'#0F4C75',color:'white',border:'none',padding:'6px 12px',borderRadius:'8px',fontSize:'11px',fontWeight:'bold'}}>🔄 Опресни</button>
                </div>
                
                {myShopProducts.length===0 ? (
                  <div style={{textAlign:'center',padding:'16px',background:'#ffeaea',borderRadius:'10px',marginTop:'10px',border:'2px solid red'}}>
                    <div style={{fontWeight:'bold',color:'red'}}>⚠️ 0 продукта</div>
                    <div style={{fontSize:'11px',marginTop:'6px'}}>В Пазар има {marketProducts.length>0?marketProducts.length:1} продукт за HALASTRA2!</div>
                    <div style={{fontSize:'10px',marginTop:'6px',textAlign:'left',background:'white',padding:'8px',borderRadius:'6px'}}>
                      Твоя shop_id: {myShopProfile.shop_id}<br/>
                      Тел: {currentUser.phone}<br/>
                      Clean: {clean(currentUser.phone)}<br/>
                      {debugMsg}
                    </div>
                    <button onClick={async()=>{
                      // Форсирай зареждане от Пазар
                      const {data}=await supabase.from('products').select('*');
                      alert(`Всички продукти в базата: ${data?.length}\nТвоя shop_id: ${myShopProfile.shop_id}\nПродукти за твоя shop: ${data?.filter((p:any)=>p.shop_id===myShopProfile.shop_id).length}\nАко е 0 - ПУСНИ RLS FIX SQL!`);
                    }} style={{marginTop:'8px',background:'red',color:'white',border:'none',padding:'8px 12px',borderRadius:'6px',fontSize:'11px'}}>🔍 Дебъг всички продукти</button>
                    <div style={{fontSize:'10px',marginTop:'8px',background:'#fff3cd',padding:'8px',borderRadius:'6px'}}>Пусни в Supabase SQL Editor:<br/>CREATE POLICY "public all products" ON products FOR ALL USING (true) WITH CHECK (true);</div>
                  </div>
                ) : (
                  myShopProducts.map((p:any)=><div key={p.id} style={{display:'flex',gap:'10px',alignItems:'center',border:'1px solid #e5e7eb',padding:'10px',borderRadius:'10px',marginTop:'8px',background:p.id===editingProduct?.id?'#fef3c7':'white'}}>{p.image_url && <img src={p.image_url} style={{width:'55px',height:'55px',objectFit:'cover',borderRadius:'8px'}}/>}<div style={{flex:1}}><b>{p.name}</b><br/><span style={{fontSize:'11px'}}>{p.description||''}</span><br/><b style={{color:'#22c55e'}}>{p.price}€</b></div><div style={{display:'flex',flexDirection:'column',gap:'4px'}}><button onClick={()=>startEditProduct(p)} style={{background:'#0F4C75',color:'white',border:'none',padding:'6px 10px',borderRadius:'6px',fontSize:'10px'}}>✏️ Промени</button><button onClick={()=>deleteProduct(p.id)} style={{background:'#FF3B30',color:'white',border:'none',padding:'4px 8px',borderRadius:'6px',fontSize:'10px'}}>🗑️</button></div></div>)
                )}
              </div>
            </div>
          )}
          {!myShopProfile && <div style={{background:'white',border:'1px dashed #22c55e',padding:'12px',borderRadius:'12px',fontSize:'12px',textAlign:'center'}}>Нямаш магазин. Тел: {currentUser.phone}<br/>Админът да те добави в 👑 Магазини.<br/>{debugMsg && <div style={{fontSize:'10px',background:'black',color:'#0f0',padding:'6px',marginTop:'6px',textAlign:'left'}}>{debugMsg}</div>}</div>}
        </div>}
      </div>
    </main>
  );
